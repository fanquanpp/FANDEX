 ...
 *
 * 功能概述：
 * 扫描 cnt-content/full 下所有 .md/.mdx 文件，提取 frontmatter 中的元数据
...
/** 文档源文件目录（共享内容源） */
const DOCS_DIR = join(MONO_ROOT, 'cnt-content', 'full');
/** 索引输出目录 */
 ...
 */
export async function walkDir(dir, exts, fn) {
  const entries = await readdir(dir, { withFileTypes: true });
  for (const entry of entries) {
    const full = join(dir, entry.name);
    if (entry.isDirectory())
      await walkDir(full, exts, fn); // 递归子目录
    else if (exts.some((ext) => entry.name.endsWith(ext))) await fn(full);
  }
}
...
export async function main(options = {}) {
  ...
  // 遍历所有 .md/.mdx 文件，提取元数据
  await walkDir(docsDir, ['.md', '.mdx'], async (filePath) => {
