...
// 偏差报备（仓库整理后数据源迁移）：
// 原：import glossaryData from '../data/glossary.json'（app 端硬编码扁平数组）
// 新：直接读取 shd-shared/metadata/glossary/*.json（三端共享元数据，按模块分文件）
// 迁移依据：消除 app-web/src/data/ 硬编码数据，统一从 shd-shared/metadata/ 实时读取
//
// 技术说明：remark 插件在 astro.config.ts 加载阶段运行，此时 astro:content 虚拟模块
// 尚未注入，无法通过 glossary-service.ts 间接调用。改用 Node.js fs 直接读取 JSON 文件，
// 既绕开 astro:content 依赖，又实现从共享元数据源实时读取。
/** 本文件所在目录（app-web/src/plugins/） */
const __dirname = dirname(fileURLToPath(import.meta.url));
/** 术语表源文件目录（shd-shared/metadata/glossary/） */
const GLOSSARY_DIR = join(__dirname, '..', '..', '..', 'shd-shared', 'metadata', 'glossary');
 ...
 * 模块初始化时一次性获取全部术语并缓存
 *
 * 从 shd-shared/metadata/glossary/ 目录读取全部模块的 JSON 文件，
 * 提取 terms[].name 字段并去重。
...
const TERM_LIST: readonly string[] = (() => {
  ...
  try {
    ...
    for (const file of files) {
      const data = JSON.parse(readFileSync(join(GLOSSARY_DIR, file), 'utf-8'));
