 ...
 *
 * 功能概述：
 * 基于 shd-shared/metadata/roadmap/learning-paths.json 共享元数据，提供职业学习路径的
 * 查询、进度聚合与推荐能力。与 progress-service 协作，结合用户当前进度生成个性化路径推荐。
 *
 * 设计原则：
 * - 单一职责：仅处理路径相关业务，不直接操作存储（通过 progress-service 读取进度）
 * - SSR 安全：纯数据查询函数可在 SSR 阶段安全调用；异步函数在 SSR 阶段返回空值
 * - 类型严格：无 any/unknown，所有类型显式声明
 * - 无循环依赖：仅依赖共享元数据、progress-service、module-service
 *
 * 偏差报备（仓库整理后数据源迁移）：
 * - 原：app-web/src/data/learning-paths.json（app 端硬编码）
 * - 新：shd-shared/metadata/roadmap/learning-paths.json（三端共享元数据源）
 * - 迁移依据：消除 app-web 硬编码数据，统一从 shd-shared/metadata/ 实时读取
 */

// 从共享元数据读取（路径：app-web/src/services/ → ../../../shd-shared/metadata/roadmap/）
import learningPathsData from '../../../shd-shared/metadata/roadmap/learning-paths.json';
