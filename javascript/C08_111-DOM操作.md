...
⚠️ **注意**：克隆不会复制事件监听器和 `data-*` 属性中通过 JS 设置的值。
...
```html
<div id="user" data-user-id="42" data-role="admin" data-last-login="2026-01-01">
  User Info
</div>
```
...
| `toggle(token, force?)` | 切换类名，`force` 为 `true` 添加，`false` 移除 |
...
`passive: true` 告诉浏览器该监听器不会调用 `preventDefault()`，浏览器可以立即开始滚动而不必等待 JS 执行完毕。
