...
⚠️ 如果手动替换了 `prototype`，需要修复 `constructor`：
...
创建一个新对象，并将其 `[[Prototype]]` 设置为 `proto`：
...
console.time('own')
for (let i = 0; i < 1e6; i++) { e.y = 1; void e.y }
console.timeEnd('own')

console.time('deep')
for (let i = 0; i < 1e6; i++) { void e.x }
