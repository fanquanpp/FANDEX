...
```sql
-- MySQL：默认限制 1000 层，超深会报错 3636，可放宽
SET SESSION cte_max_recursion_depth = 10000;

-- PostgreSQL：没有专门的递归深度参数（受内存与 max_stack_depth 约束）
-- 深度防护要靠查询自身携带的 level/depth 列：
WITH RECURSIVE t AS (
    SELECT 1 AS n
    UNION ALL
    SELECT n + 1 FROM t WHERE n < 100   -- 在递归成员里显式设上限
)
SELECT * FROM t;
```

另外，把递归成员的 `UNION ALL` 换成 `UNION`（去重）可以让重复行不再增长，
对有环数据也是一种天然的止涨手段。
...
SELECT
    ...
    content,
    depth,
    -- 缩进展示：PostgreSQL 用 || 拼接；MySQL 请改写为 CONCAT(REPEAT('  ', depth), content)
...
- 自顶向下查子孙、自底向上查祖先，只是把递归成员的连接方向反过来；评论树用
  `LPAD` 编码的 sort_path 排序即可得到"深度优先"的展示顺序。
- 进阶注意：MySQL 有 `cte_max_recursion_depth`（默认 1000）兜底，PostgreSQL 只能
  靠查询自带的 level 上限或 `UNION` 去重止涨；图数据必须显式做环检测（路径数组/
  FIND_IN_SET）；单表深层级、写多读多的场景先评估闭包表等物化方案，
  递归 CTE 是灵活的默认解，不总是最快的解。
