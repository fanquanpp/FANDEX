...
- 最新 ISO 标准为 SQL:2023（ISO/IEC 9075:2023），新增属性图查询（SQL/PGQ）、JSON 增强等。
- MySQL、PostgreSQL、SQLite 等各自实现标准子集并扩展方言；学习时以标准语法为骨架，部署时以具体数据库文档为准。
- 本模块前 10 篇左右的入门文档为必读核心，其后按需查阅。
...
SQL-86 ──→ SQL-89 ──→ SQL-92 ──→ SQL:1999 ──→ SQL:2003 ──→ SQL:2006 ──→ SQL:2008 ──→ SQL:2011 ──→ SQL:2016 ──→ SQL:2023
 奠基      微修订     重大修订    对象扩展     窗口函数/XML   SQL/XML      MERGE等      时态表        JSON        属性图查询
```
...
- **窗口函数**：`OVER()`、`RANK()`、`ROW_NUMBER()` 等（经 OLAP 附加修订引入，SQL:2003 并入主标准）
- **用户自定义类型**（UDT）
...
- **JSON 增强**：更多 JSON 构造、查询简写与谓词（`IS JSON` 等）
- **UNIQUE 约束的 NULL 处理**：`UNIQUE NULLS NOT DISTINCT` 等新选项
- SQL:2023 是截至目前最新的完整标准版本，数据库厂商仍在逐步跟进实现
...
| **易用性**       | 高（安装即用，资料最多） | 中高（新手门槛已大幅降低） | 中（配套工具完善） | 中（体系庞大，门槛高） |
| **扩展性**       | 插件机制有限 | 极强（PostGIS、pgvector 等扩展生态） | 中 | 中 |
| **性能（OLTP）** | 优秀（读密集场景） | 优秀（高并发写更稳） | 优秀 | 优秀 |
| **分析能力**     | 一般 | 强（窗口函数、CTE、并行查询） | 强 | 强 |
| **JSON 支持**    | 好（JSON 类型 + 函数） | 极好（JSONB 二进制存储可建索引） | 中（JSON 函数） | 中（21c 起原生） |
| **全文搜索**     | 内置 FULLTEXT（功能较弱） | 内置 tsvector（功能较强） | 内置（中等） | Oracle Text |
| **社区生态**     | 极大 | 大且增长快 | 中（微软技术栈） | 中（商业支持） |
| **运维成本**     | 低         | 低                  | 中         | 高     |
...
```mermaid
flowchart TD
    T0["项目需要数据库"] --> T1{"预算有限 / 开源优先？"}
    T1 -- 是 --> T2{"需要复杂查询 / 分析 / JSON / GIS？"}
    T2 -- 是 --> T3["PostgreSQL"]
    T2 -- 否 --> T4{"简单 Web 应用 / 高并发读？"}
    T4 -- 是 --> T5["MySQL"]
    T4 -- 否 --> T3
    T1 -- 否 --> T6{"微软技术栈 / 企业级 Windows 环境？"}
    T6 -- 是 --> T7["SQL Server"]
    T6 -- 否 --> T8{"大型企业 / 关键业务 / 极致可靠性？"}
    T8 -- 是 --> T9["Oracle"]
    T8 -- 否 --> T3
    T0 --> T10{"需要嵌入式 / 本地存储？"}
    T10 -- 是 --> T11["SQLite"]
    T0 --> T12{"需要云原生 / 分布式？"}
    T12 -- 云托管 --> T13["Amazon Aurora"]
    T12 -- 全球分布式 --> T14["CockroachDB / Spanner 类"]
```
