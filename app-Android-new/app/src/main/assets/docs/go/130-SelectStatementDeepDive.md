...
- [channel 底层原理](/go/150-ChannelPrinciple)：select 的每个 case 都是一次 channel 收发尝试，理解阻塞队列才能理解分支调度。
- [context 详解](/go/140-ContextDetailed)：`ctx.Done()` 返回的关闭型 channel 是 select 退出分支的标准来源。
- [Go 并发编程](/go/120-GoConcurrentProgramming)：goroutine 与 channel 的基本用法是本篇的直接前置。
