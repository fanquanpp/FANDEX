...
- [竞态检测与原子操作](/go/190-RaceDetectionAtomic)：建议先完成前一篇的学习
...
- 新增 `go work vendor`：把工作区内模块的依赖也纳入 vendor 目录，离线构建覆盖多模块仓库
- vendor 模式下 `go build` 默认使用 `-mod=vendor`，行为与单模块一致
