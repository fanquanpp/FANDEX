...
related:
  ...
  - 'python/370-PythonAndCryptography'
...
- [Python 与测试](/python/750-PythonTest)：建议先完成前一篇的学习
...
class Settings(BaseSettings):
    ...
    def validate_secret(self):
        if self.app_env == 'production' and self.secret_key.get_secret_value() == 'change-me':
...
# lifespan：启动时验证配置（on_event("startup") 的现代替代，后者已弃用）
@asynccontextmanager
...
# 启动时验证配置已通过上面的 lifespan 完成：生产 + debug 组合会直接拒绝启动
```
