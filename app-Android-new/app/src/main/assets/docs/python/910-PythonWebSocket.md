...
- [Python 与 gRPC](/python/940-PythonGrpc)：建议先完成前一篇的学习

## 学习目标

- 掌握「什么是 WebSocket」的核心机制、典型用法与常见陷阱
...
# 应用启动时启动 Redis 订阅（lifespan 是 on_event("startup") 的现代替代，
# 后者自 FastAPI 0.93 起已弃用）
from contextlib import asynccontextmanager

@asynccontextmanager
async def lifespan(app: FastAPI):
    task = asyncio.create_task(redis_subscriber())  # 启动时拉起订阅任务
    yield
    task.cancel()  # 关闭时取消，避免任务泄漏
    try:
        await task
