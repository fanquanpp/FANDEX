...
- [React 与 Monorepo](/react/380-ReactMonorepo)：建议先完成前一篇的学习
...
| **2023 年 3 月** | React Labs 博客首次公开介绍编译器方向（代号 React Forget） |
| **2024 年 2 月** | React 19 Beta 集成 Compiler，发布 `babel-plugin-react-compiler` |
| **2024 年 5 月** | React Conf 2024 公开 Compiler 进展，canary 版本先行试用 |
| **2024 年 12 月** | React 19 GA，Compiler 持续以 canary/RC 形态迭代 |
| **2025 年 4 月** | React Compiler 进入 RC 阶段；Next.js 15.2+ 支持 `reactCompiler: true` 配置 |
| **2025 年 10 月** | React Compiler v1.0 正式稳定（2025-10-07），覆盖绝大多数 React 模式 |
...
Vercel 在 Next.js 15 时代深度接入 Compiler：

**背景**：Vercel Dashboard 是复杂的交互式应用，手动记忆化代码占比 15%。

**方案**：
- Next.js 通过 `experimental.reactCompiler` 配置按需启用（opt-in，并非默认开启）
...
**基本写法：Next.js 配置启用（opt-in）**
`module.exports = { experimental: { reactCompiler: true } }`
```js
// next.config.js — Next.js 15.2+ 支持，需显式开启
module.exports = {
