...
### 3.3 Chosen-Prefix 碰撞（2020）

「SHA-1 is a Shambles」研究进一步实现了**选择前缀碰撞**（约 $2^{63.4}$，
租用云 GPU 在数万美元预算内可复现）：攻击者可以预先构造两份**开头不同、
结尾任意不同、哈希相同**的文档（如两份金额不同的合同）。这使 SHA-1
在任何「防抵赖/防伪造」场景的风险从理论变为工程可行。

**结论**：SHA-1 禁止用于安全场景；Git 等版本系统仍以 SHA-1 为对象名，
但依赖的是其第二原像抗性并已有增强（碰撞检测、向 SHA-256 的迁移计划），
安全敏感的新系统一律选 SHA-256 及以上。
...
- **BLAKE2/BLAKE3**：非 NIST 标准但广泛使用的高性能哈希（BLAKE3 支持并行与
