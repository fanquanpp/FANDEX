...
flowchart TD
    ...
    Inno --> DW[Doublewrite]
    Inno --> UT[Undo Tables]
    Inno --> FS[文件系统层<br/>.ibd / ibdata1 / #innodb_redo]
```
...
-- innodb_redo_log_capacity: redo 总容量（8.0.30+，在线可调；默认 100MB）
-- innodb_log_buffer_size: 日志缓冲区大小
-- （innodb_log_file_size / innodb_log_files_in_group 已在 8.4 移除，勿再使用）

-- MySQL 8.0.30+ 动态调整 redo log 总容量
...
-- innodb_max_undo_log_size: undo 表空间触发截断的阈值
-- innodb_undo_log_truncate: 在线截断开关（8.0 默认开启）
-- （innodb_undo_tablespaces 已废弃：8.0.14 起固定默认 2 个独立 undo 表空间）
