...
| 陷阱 | 原因 | 正确做法 |
| --- | --- | --- |
| `<Transition>` 包裹多个元素 | Transition 只支持单一元素或组件 | 只放一个根节点；列表动画改用 `<TransitionGroup>` |
| 切换相同标签不触发动画 | Vue 复用了同标签元素，没有替换发生 | 给元素加不同的 `key`（`<div :key="state">`） |
| CSS animation 写了却不生效 | Transition 默认监听 `transitionend`，动画用的是 `animation` | 加 `type="animation"`，或显式 `:duration` |
| JS 钩子只写 `@enter` 不调用 done | 没有完成信号，Vue 立即结束过渡 | 用 `:css="false"` 并在动画结束时调用 `done()` |
| TransitionGroup 忘了 `position: absolute` | 离开项仍占位，move 动画错乱 | `.list-leave-active { position: absolute }` 让离开项脱离文档流 |
| 模式写反：`mode="in-out"` 用于路由切换 | 新页面先进入、旧页面后离开，出现叠影 | 路由/互斥内容切换用 `mode="out-in"` |
