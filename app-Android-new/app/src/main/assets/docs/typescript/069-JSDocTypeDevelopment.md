...
比 `import()` 内联类型更整洁，且 `@import * as ns from './x.js'` 支持命名空间形式。另有 `@satisfies`（对齐 `satisfies` 操作符，见 `typescript/052-SatisfiesOperator`）。
...
`tsc` 从带 JSDoc 的 JS 源码直接产出声明文件——即"JSDoc 写类型、tsc 出 dts"的库发布路线。配合 TS 5.5 的 `isolatedDeclarations`（见 `typescript/050-TypeScript5xNewFeatures`），产出更快、更可预测。发布卫生（exports/files 字段）见 `javascript/057-PackageJsonEngineeringDeepDive`。
