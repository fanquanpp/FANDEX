...
updated: '2026-09-12'
related:
  - 'typescript/040-TSBasicsVariablesAndTypes'
...
同一个函数对不同的参数组合给出不同的"门面签名"。它依赖"签名 + 实现"的分离写法，属于进阶内容，见 `typescript/220-FunctionGeneric` 中的完整展开。
...
2. 开启 `noImplicitThis`（见 `typescript/350-TypeScriptEngineeringConfig`），观察普通函数版本是否被编译器警告。
