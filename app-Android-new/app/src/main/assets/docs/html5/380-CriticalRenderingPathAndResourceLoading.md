...
>
> 前置要求：本篇是 005"性能优化"小节的深化版。需要先理解资源加载与响应式图片（`html5/018`）、CSS 盒模型（`css/050-CSS3BoxModelDetailed`）、浏览器事件循环（`javascript/027`）；移动端性能关联 `html5/034`。建议学完 CSS 性能优化（`css/540-CSSPerformanceOptimizationDetailed`）后再做完整实践。
...
4. 完整的缓存与 HTTP 语义见 `networking/010-NetworkBasicsAndProtocol` 与后端模块的响应头配置。
...
测量方式：Lighthouse（F12 → Lighthouse → Generate report）一键生成；详细教程见 `javascript/510-CoreWebVitalsAndPerformanceMetrics`。本文的每一条优化都可以映射到指标：内联 CSS 优化 FCP/LCP，图片宽高优化 CLS，脚本 defer 优化 TBT。
...
- CSS 侧：`css/540-CSSPerformanceOptimizationDetailed` 的渲染路径优化清单；
- 概念基础：`css/050-CSS3BoxModelDetailed`（盒模型）、`javascript/290-EventLoop`（事件循环与主线程）；
- 指标验证：`javascript/510-CoreWebVitalsAndPerformanceMetrics` 中 LCP/CLS/TBT 的测量；
- 资源加载：`html5/140-ImagesAndResponsiveImages` 中图片的优先级与懒加载；
- 移动性能：`html5/360-ViewportConfigMobileFirst` 中移动优先与视口策略；
...
`css/540-CSSPerformanceOptimizationDetailed` 与 `javascript/510-CoreWebVitalsAndPerformanceMetrics` 形成闭环验证。
