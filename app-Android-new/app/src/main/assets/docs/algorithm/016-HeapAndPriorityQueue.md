...
> **跨模块引用**：堆作为完全二叉树的应用参见 [树](/algorithm$2；堆排序与快排、归并的对比参见 [排序算法](/algorithm$2；Dijkstra/Prim 算法中堆的应用参见 [图算法](/algorithm$2；堆在流式数据 Top-K 中的应用参见 [查找算法](/algorithm$2；堆作为优先队列与栈/队列的对比参见 [栈与队列](/algorithm$2。
