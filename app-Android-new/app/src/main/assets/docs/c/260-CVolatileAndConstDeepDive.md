...
- [变量与常量](/c/050-VariableConstant)：变量声明、作用域与初始化是理解限定符叠加的基础。
- [位运算与位域](/c/070-BitwiseBitField)：读取状态寄存器、解析标志位都要用到按位与、移位等操作。
- [C 原子操作与内存模型](/c/380-AtomicAndMemoryModel)：理解 volatile 与真正线程安全的差距，需要内存模型的背景。
