...
安装 JDK 后（步骤见 [Java 开发环境搭建](/java/020-JavaOverviewDevEnv)），保存文件 `Hello.java`，在终端执行：
...
进入 [Java 概述与开发环境](/java/020-JavaOverviewDevEnv) 系统学习；本仓库另有 kotlin 模块——它与 Java 运行于同一个 JVM，语法更现代，学完 Java 基础后交叉阅读收益极大。
