...
- [C++ 是什么：从 C 到高性能泛型编程](/cpp/010-WhatIsCpp)：C 与 C++ 的关系、能力版图、第一个现代程序。
- [C++ 基础语法](/cpp/030-CppBasicSyntax)：变量、控制流、函数的现代化写法。
- [C++ 类型系统](/cpp/040-CppTypeSystem)：基本类型、引用与值类别，是移动语义的前置。
...
1. 精读 [右值引用与移动语义](/cpp/090-RvalueReferenceMoveSemantics) 与 [完美转发与引用折叠](/cpp/110-PerfectForwardingReferenceCollapse)，补齐值类别与转发的完整理论。
