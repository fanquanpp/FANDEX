...
**纪律**：FinalizationRegistry 的回调时机由 GC 决定，不可用于业务清理逻辑；需要确定释放的资源改用显式资源管理（`using` 与 `Symbol.dispose`，见 `javascript/620-ExplicitResourceManagement`）。弱引用只解决"不阻止回收"，不解决"何时释放"。
...
- 闭包：`javascript/080-FunctionScopeClosure`；
- 泄漏排查：`javascript/370-MemoryLeakTroubleshoot`；
- 集合弱引用：`javascript/230-MapSetWeakMapWeakSet`；
- 显式资源管理：`javascript/620-ExplicitResourceManagement`。
