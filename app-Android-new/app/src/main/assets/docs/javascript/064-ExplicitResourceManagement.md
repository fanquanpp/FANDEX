...
`using` **不替代 GC**：它管理的是"显式资源"，内存回收仍由 GC 负责。两者的分工是——内存交给 GC，带副作用的稀缺资源交给 `using`。此前只能靠 `FinalizationRegistry` 兜底的场景（见 `javascript/037-MemoryManagementAndGarbageCollection`），现在应优先改写为 `using`。
