...
>
> 方向提醒：SVG 的 y 轴向下，因此**正角度是顺时针**旋转（这与数学课本里 y 轴向上的坐标系正好相反，但与屏幕直觉一致）。
...
多个变换用空格分隔，构成变换列表。矩阵层面它们按书写顺序依次**左乘**：`transform="A B C"` 等价于矩阵 `A × B × C`，作用到具体坐标点时是"**从右往左**"依次生效（最右边的变换最先接触元素自己的坐标），这一点与 CSS transform 一致。

一个不易失真的类比：变换列表像给元素**层层套坐标系**，每往右写一个函数，就往里再套一层，因此最右边的函数离元素最近、最先作用到它自身的坐标。
