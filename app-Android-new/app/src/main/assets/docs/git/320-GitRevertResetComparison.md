...
`git reset` 将当前分支指针移动到指定提交（HEAD 随之移动），根据模式决定如何处理暂存区和工作区。与 revert 的本质区别：**被放弃的提交从分支历史中消失**（对象本体在 reflog 过期前仍可找回）。

```text
原始历史：A──B──C──D（HEAD → D）
reset B 之后：A──B（HEAD → B），C、D 不再被分支引用
```
...
# d4e5f6e HEAD@{1}: commit: feat: add feature D
# 8f7a6b5 HEAD@{2}: commit: feat: add feature C
```
...
# 恢复到该提交
git reset --hard d4e5f6e
# 或者创建新分支指向它
git branch recovered d4e5f6e
```
