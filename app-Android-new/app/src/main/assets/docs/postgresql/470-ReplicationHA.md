...
# synchronous_commit 参数（按安全度从高到低）:
# remote_apply  — 等待备库回放完成, 查询立即可见（最安全, 延迟最高）
# on            — 本地刷盘 + 等待同步备库将 WAL 刷盘（默认）
# remote_write  — 等待备库收到 WAL 并写入 OS 缓存（尚未刷盘）
# local         — 仅本地刷盘即确认（不等待同步备库）
# off           — 异步提交, 不等待本地刷盘（性能最高, 可能丢失最近提交）
```
...
1. 不复制 DDL（需手动同步表结构）
2. 不复制序列值（需手动同步）
3. 不复制大对象（普通 BYTEA 列可复制）
4. TRUNCATE 自 PostgreSQL 11 起支持复制（需发布包含 truncate 特性）
5. UPDATE/DELETE 需要复制标识（通常是主键）来定位行, 否则报错
6. 同一表不能有多个订阅源
...
```bash
# 全量备份（推荐 plain 目录格式, 便于后续增量引用）
pg_basebackup -h 192.168.1.10 -U replicator \
  -D /backup/full -Fp -Xs -P

# 增量备份（PostgreSQL 17+）
# --incremental 指向参考备份的 backup_manifest 清单文件
pg_basebackup -h 192.168.1.10 -U replicator \
  -D /backup/incremental \
  -Fp -Xs -P \
  --incremental=/backup/full/backup_manifest

# 合并增量备份: 按从旧到新的顺序列出全量与全部增量
# -o 输出目录为必填参数, 产物是"合成全量备份", 可直接用于恢复
pg_combinebackup /backup/full /backup/incremental \
  -o /backup/merged

# 恢复前可用 pg_verifybackup 依据清单校验备份完整性
```
...
-- 对象级审计: 通过 pgaudit.role 主角色 + GRANT 实现
-- 1. 创建"审计主角色"并在配置中指定
--    postgresql.conf:  pgaudit.role = 'auditor'
-- 2. 把希望被审计的对象权限授予该角色:
GRANT SELECT ON orders TO auditor;
-- 此后所有对 orders 的 SELECT 都会产生 AUDIT: OBJECT 日志
-- 注意: pgaudit 没有名为 pgaudit.audit_object() 的 SQL 函数
```
