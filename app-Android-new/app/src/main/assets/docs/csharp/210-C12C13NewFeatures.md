...
- [C#与最小 API](/csharp/300-CSharpAPI)：建议先完成前一篇的学习

## 学习目标

- 掌握「一、概述」的核心机制、典型用法与常见陷阱
- 掌握「二、历史演进」的核心机制、典型用法与常见陷阱
- 掌握「三、核心概念」的核心机制、典型用法与常见陷阱
- 掌握「四、工作原理」的核心机制、典型用法与常见陷阱
- 掌握「五、快速上手」的核心机制、典型用法与常见陷阱
...
| C# 14 | 2025.11 | .NET 10 | 扩展成员（正式）、`field` 关键字、null 条件赋值 |
...
4. **留意语言版本线**：扩展成员、`field` 关键字在 C# 13 时代是预览特性，已在 C# 14（.NET 10）正式发布；生产项目建议将 `LangVersion` 与目标框架对齐，避免长期依赖 `<Features>preview</Features>`。
5. **阅读官方文档与 RFC**：C# 语言设计是开放的，所有特性讨论在 [csharplang](https://github.com/dotnet/csharplang) 仓库公开，阅读设计讨论能深入理解特性动机。

### 12.5 后续版本展望

C# 14（.NET 10，2025.11）已正式落地，除本章覆盖的 C# 12/13 特性外，它还带来了：扩展成员（`extension` 块，统一扩展方法与扩展属性）、`field` 关键字（属性后备字段访问）、null 条件赋值（`obj?.Prop = value`）、`nameof` 支持非绑定泛型、`Span<T>` 更多隐式转换、简单 lambda 参数修饰符、partial 事件与构造函数、用户定义复合赋值运算符。细节见本模块[C# 14 新特性](/csharp/220-CSharp14NewFeatures)一章。下一版本 C# 15 计划随 .NET 11（2026.11）发布。
