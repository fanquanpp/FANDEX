...
- [C# 基础语法](/csharp/030-CSharpBasicSyntax)：方法、循环与集合遍历的基本写法。
- [C# LINQ 与函数式编程](/csharp/110-CSharpLINQFunctionalProgramming)：LINQ 运算符大量基于迭代器实现，是本篇的应用背景。
- [LINQ 延迟执行与立即执行](/csharp/130-LINQDeferredImmediate)：延迟执行在本篇被追溯到语言层的实现机制。
