 ...
 *
 * 设计原则：
 * - 数据模型使用不可变 data class，字段提供默认值以兼容反序列化缺失字段
 * - 状态机使用 sealed class，保证 when 分支穷举性，避免遗漏分支
 * - 相比旧端使用 Gson + @SerializedName，新端统一改用 kotlinx.serialization，
 *   通过 @SerialName 注解显式映射 GitHub API 返回的 snake_case JSON 字段
 */
