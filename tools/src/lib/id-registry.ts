 ...
 *
 * 文件位置：
 * - id-registry.json 位于 FANDEX/id-registry.json（仓库根，git 跟踪）
 * - 工具链通过相对路径读写
 */
...
/** id-registry.json 默认路径（FANDEX/id-registry.json） */
const DEFAULT_REGISTRY_PATH = resolve(__dirname, '..', '..', '..', 'id-registry.json');
 ...
 * 加载 id-registry
 *
 * 输入：可选的注册表文件路径（默认 FANDEX/id-registry.json）
 * 输出：IdRegistry 对象
 * 流程：
 * 1. 读取文件（不存在则返回空注册表）
 * 2. JSON 解析
