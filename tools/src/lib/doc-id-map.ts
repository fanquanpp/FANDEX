 ...
 *
 * 文件位置：
 * - doc-id-map.json 位于 FANDEX/doc-id-map.json（仓库根，git 跟踪）
 * - 与 id-registry.json 同级
 *
 * 数据结构：
 * {
 *   "map_version": "1.0.0",
...
/** doc-id-map.json 默认路径（FANDEX/doc-id-map-{type}.json） */
function getDefaultMapPath(manifestType: ManifestType): string {
  return resolve(__dirname, '..', '..', '..', `doc-id-map-${manifestType}.json`);
}
