 ...
 * 1. 生成 Ed25519 密钥对（私钥保存到 ~/.fandex/keys/，公钥副本保存到 tools/keys/）
 * 2. 创建空 id-registry.json
 * 3. 扫描 content/full/ 与 content/mobile/
 * 4. 批量分配所有模块 ID（english_short 来自目录名，name 来自 desktop/metadata/modules.json）
 * 5. 批量分配所有文档 ID 并写入 doc-id-map-full.json / doc-id-map-mobile.json
 *
 * 设计目的：
...
/** FANDEX 仓库根目录（tools/src/ → tools/ → FANDEX/） */
const FANDEX_ROOT = resolve(__dirname, '..', '..');

/** 默认模块元数据文件路径（desktop/metadata/modules.json） */
const DEFAULT_MODULES_META = join(FANDEX_ROOT, 'desktop', 'metadata', 'modules.json');

/** 仓库内公钥副本路径（tools/keys/public-key.hex，可入库） */
...
async function processManifestType(
  ...
  const contentDir = join(FANDEX_ROOT, 'content', manifestType);
