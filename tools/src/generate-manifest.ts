 ...
 *
 * 功能概述：
 * 扫描 content/full/ 或 content/mobile/ 目录，结合 id-registry 与 doc-id-map，
...
/** 默认内容目录（content/full/ 或 content/mobile/） */
function defaultContentDir(type: ManifestType): string {
  return join(FANDEX_ROOT, 'content', type);
}
...
async function main(): Promise<void> {
  ...
  program
    ...
    .description('扫描 content/ 目录生成未签名 manifest')
    .option(
      '-t, --type <type>',
    ...
    .option(
      '--doc-id-map <path>',
      'doc-id-map-{type}.json 路径（默认 FANDEX/doc-id-map-{type}.json）',
    )
