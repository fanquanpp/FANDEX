 ...
 *
 * 关键配置说明：
 * - 部署目标：Tauri 桌面端（基础路径 /，通过 tauri://localhost 加载）
 ...
 *
 * 环境变量：
 * - BASE_PATH：构建基础路径，桌面端默认 /（tauri://localhost 协议加载）
 */
...
import { rehypeLazyImages } from './src/lib/remark/rehype-lazy-images'; // 图片懒加载 + 防 CLS + 响应式 srcset
import { remarkAdmonition } from './src/lib/remark/remark-admonition'; // 自定义提示块（:::note、:::tip 等）
import { createRemarkMermaid } from './src/lib/remark/remark-mermaid'; // Mermaid 代码块构建时预渲染为 SVG
import { createRemarkTermLink } from './src/lib/remark/remark-term-link'; // 术语预编译标记 [[term]] → 带 tooltip 链接

/** 项目根目录（desktop 子项目根，用于定位 metadata/glossary/ 等项目级资源） */
 ...
 * 2. 默认 /（Tauri 桌面端通过 tauri://localhost/ 协议加载前端资源）
 *
 * 注意：Tauri 桌面端通过 tauri://localhost/ 协议加载前端资源，
 *       若 base path 非根路径，所有 CSS/JS 资源会 404 导致白屏。
 */
...
export default defineConfig({
  ...
  vite: {
    ...
    // 允许 Vite 访问单仓库根与 shd-shared 目录的资源
    // （modules.ts 导入 shd-shared/metadata/modules.json，content.config.ts 引用 cnt-content/full）
    server: {
      fs: {
        allow: [projectRoot, resolve(projectRoot, '..')],
      },
    },
    // 解析别名：将 @modules 指向单仓库根 shd-shared/metadata/modules.json
    // 仓库整理后 metadata 已迁移至 shd-shared/metadata/，与三端共享
    resolve: {
      alias: {
        '@modules': resolve(projectRoot, '..', 'shd-shared', 'metadata', 'modules.json'),
        // cheatsheets 数据三端共享，MDX 中通过 @/data/cheatsheets/<name>.json 引用
