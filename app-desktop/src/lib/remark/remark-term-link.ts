 ...
 *
 * 数据源：
 * - 默认从 shd-shared/metadata/glossary/<module>.json 读取术语表
 * - 解析 JSON 中的 terms 数组（字段：name、definition、slug）
 * - 模块级缓存，构建一次后复用
 ...
 * - 原 english 列已合并到 name 字段，因此不再生成英文术语条目
 * - 原 slug 字段（页面级 javascript/glossary）不足以定位单个术语，
 *   改为根据 name 字段动态生成术语级 slug
 *
 * 匹配规则：
...
export interface RemarkTermLinkOptions {
  ...
  /** 项目根目录（用于定位 glossary 目录，默认推断为 app-desktop） */
  baseDir?: string;
  /** 每篇文档最多标记术语数（默认 100） */
  maxMatches?: number;
}
...
/** glossary 目录相对于 app-desktop 项目根的相对路径 */
const _GLOSSARY_DIR_RELATIVE = '../shd-shared/metadata/glossary';
...
/** glossary JSON 文件术语条目的运行时结构 */
interface GlossaryJsonTerm {
  name: string;
  definition: string;
  /** JSON 数据源中的 slug 为页面级（如 javascript/glossary），此处不直接使用 */
  slug?: string;
}

/** glossary JSON 文件的顶层结构 */
interface GlossaryJson {
  moduleId?: string;
  terms?: GlossaryJsonTerm[];
}

/**
 * 从单个 glossary JSON 文件构建术语条目
 *
 * 输入：文件路径、模块 ID（fallback）
 * 输出：术语条目数组
 * 流程：
 * 1. 同步读取 JSON 文件内容
 * 2. 解析 JSON 获取 moduleId 与 terms 数组
 * 3. 遍历 terms，将每条转为 TermData（slug 由 name 动态生成）
 *
 * @param filePath - JSON 文件绝对路径
 * @param module - 模块 ID（fallback，优先使用 JSON 内 moduleId）
 * @returns 术语条目数组
 */
function parseGlossaryJsonFile(filePath: string, module: string): Array<TermData> {
  let raw: string;
  try {
  ...
  let data: GlossaryJson;
  try {
    data = JSON.parse(raw) as GlossaryJson;
  } catch {
    console.warn(`[remark-term-link] JSON 解析失败: ${filePath}`);
    return [];
  }
  ...
  for (const term of terms) {
    ...
    entries.push({
      ...
      slug: `/glossary/${resolvedModule}/#${slugify(name)}`,
    });
  }
...
function buildGlossaryMap(glossaryDir: string): GlossaryMap {
  ...
  try {
    jsonFiles = readdirSync(glossaryDir, { withFileTypes: true })
  ...
  for (const fileName of jsonFiles) {
    const module = fileName.replace(/\.json$/, '');
    const glossaryFile = join(glossaryDir, fileName);

    const entries = parseGlossaryJsonFile(glossaryFile, module);
 ...
 * 获取术语映射表（带缓存）
 *
 * 输入：项目根目录路径（app-desktop）
 * 输出：术语映射表
 * 流程：首次调用时构建，后续直接返回缓存
 *
 * @param baseDir - 项目根目录路径（app-desktop）
 * @returns 术语映射表
 */
function getGlossaryMap(baseDir: string): GlossaryMap {
  ...
  const glossaryDir = join(baseDir, _GLOSSARY_DIR_RELATIVE);
  cachedGlossary = buildGlossaryMap(glossaryDir);
 ...
 * 推断默认项目根目录
 *
 * 默认基于当前文件位置推断 app-desktop 项目根：
 * 当前文件位于 app-desktop/src/lib/remark/remark-term-link.ts
 * 推断 app-desktop 项目根为 ../../..
 *
 * 偏差报备（模块化优化）：
 * - 原：app-desktop/src/lib/remark-term-link.ts，回退两级到 app-desktop/
 * - 新：app-desktop/src/lib/remark/remark-term-link.ts，需回退三级到 app-desktop/
 *
 * @returns app-desktop 项目根目录绝对路径
 */
 ...
 *
 * @param baseDir - app-desktop 项目根目录路径
 * @returns 术语映射表
 */
