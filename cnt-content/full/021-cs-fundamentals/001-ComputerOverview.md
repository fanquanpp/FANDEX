...
> 跨模块引用：[C语言](/c/001-CZeroBasisStart)的指针操作直接穿透了语言运行时层到达指令集层，这是C语言"接近硬件"的本质原因。[C++](/cpp/001-WhatIsCpp)的RAII则在语言运行时层提供了资源管理的抽象。
...
> 跨模块引用：[离散数学](discrete-math)中的自动机理论是状态机主线的数学基础。[Java](/java/001-WhatIsJava)线程的状态模型与OS进程状态机直接对应。
