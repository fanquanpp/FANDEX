...
> 跨模块引用：[C语言](/c$2的函数指针特性直接利用了冯诺依曼体系中"代码即数据"的本质。[操作系统](os)的虚拟内存管理通过MMU在冯诺依曼体系上实现了地址空间的隔离。
...
> 跨模块引用：[编译原理](compiler)的代码生成阶段需要根据ISA选择合适的寻址模式和指令调度。[C语言](/c$2的指针算术直接映射到基址偏移寻址模式。
...
> 跨模块引用：[操作系统](os)的内存管理建立在虚拟内存机制之上。[C语言](/c$2的指针解引用触发完整的地址翻译链路。[C++](/cpp$2的智能指针在虚拟内存之上增加了语义层。
...
> 跨模块引用：[C++](/cpp$2的内存模型（memory_order_relaxed/acquire/release/seq_cst）直接映射到硬件一致性模型。[Java](/java$2的volatile关键字在x86上无需额外屏障，但在ARM上需要dmb。
