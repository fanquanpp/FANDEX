...
> 跨模块引用：[C++](/cpp/010-WhatIsCpp)的内存模型（memory_order_relaxed/acquire/release/seq_cst）直接映射到硬件一致性模型。[Java](/java/010-WhatIsJava)的volatile关键字在x86上无需额外屏障，但在ARM上需要dmb。
