...
thiserror 适合**库**：定义精确的错误枚举，让调用方按类型分支处理。而**应用程序**（CLI、服务二进制）通常不关心错误的具体分类，只想拿到人类可读的信息继续向上传播。社区惯例是：**库用 thiserror，应用用 anyhow**。
...
| 同一项目的不同分层 | 业务库用 thiserror，main/装配层用 anyhow 汇总 |
