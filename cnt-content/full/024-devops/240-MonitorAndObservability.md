...
route:
  ...
  routes:
    - matchers: # match/match_re 已弃用（Alertmanager 0.22+），统一用 matchers
        - severity = "critical"
      receiver: 'pagerduty'
      repeat_interval: 15m
...
```yaml
# Filebeat 配置（7.16+ 推荐使用 filestream 输入，旧的 log 输入已废弃）
filebeat.inputs:
  - type: filestream
    id: app-logs
    enabled: true
...
> 注意：Promtail 已进入维护模式并被官方弃用，Loki 官方推荐迁移到
> **Grafana Alloy**（基于 OpenTelemetry Collector 的发行版，兼可采集指标/日志/链路）。
> 以下 Promtail 配置仍可运行，便于对照旧环境；新部署建议直接使用 Alloy 的
> `loki.source.file` 等组件。
...
services:
  jaeger:
    image: jaegertracing/all-in-one:1.62 # Jaeger v1 全家桶；v2（基于 OTel Collector）为新主线
    environment:
