...
- [数据类型转换](/java/050-DataTypeConversion)：基本类型与包装类型之间的自动装箱、自动拆箱语法。
- [运算符与表达式](/java/080-OperatorExpression)：`==` 对基本类型比数值、对引用类型比地址的双重含义。
- [equals 与 hashCode 契约](/java/200-EqualsHashCodeContract)：包装类 `equals()` 的比较依据与契约。
