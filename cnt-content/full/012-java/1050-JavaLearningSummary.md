...
- [Java 是什么：一次编写、到处运行的企业级语言](/java/010-WhatIsJava)：理解 Java 的定位、字节码与 JVM 的运行模型，这是全部讨论的地基。
- [面向对象编程](/java/150-OOP)：类、封装、继承与多态是 Java 语法体系的主干，后续所有主题都建立在其上。
- [集合框架详解](/java/210-CollectionFrameworkDetailed)：集合是 Java 工程代码中出现频率最高的 API 体系，也是函数式与并发主题的前置。
