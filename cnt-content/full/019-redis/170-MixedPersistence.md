...
> 7.0 multi-part AOF 之后：上述「RDB 开头 + AOF 增量」拆成了两个文件——
> RDB 快照独立为 `*.base.rdb`（base），增量命令在 `*.incr.aof`（incr），
> 由 manifest 清单串联，本质与混合持久化思想一致，加载顺序不变
> （先 base 后 incr）。4.x~6.x 才是真正的「单文件混合」。
