...
```redis
-- Redis 7.0 引入 Function：作为 EVAL/SCRIPT 的演进（并非移除 EVAL，
--    两者并存；新代码官方推荐 Function，可持久化、有名字、好管理）
...
```redis
-- 传统 Lua 脚本（每次传输脚本文本；SCRIPT FLUSH 或重启后需重新加载）
...
**基本写法：地理多边形搜索（7.2）**
`GEOSEARCH <key> FROMMEMBER <成员> BYRADIUS ... | ...`
```redis
