...
- [数据类型详解](/c/040-DataTypeDetailed)：建议先完成前一篇的学习
...
flowchart TD
    ...
    T1["变量"]
    T2["类型 (type)"]
    T3["基本类型 (int, char, float, double, void)"]
    T4["派生类型 (指针, 数组, 函数, 结构体, 联合体)"]
    T5["限定符 (const, volatile, restrict, _Atomic)"]
    T6["存储期 (storage duration)"]
    T7["自动 (auto, register)"]
    T8["静态 (static, extern)"]
    T9["线程 (_Thread_local)"]
    T10["动态 (malloc/calloc/realloc/free)"]
    T11["作用域 (scope)"]
    T12["块作用域"]
    T13["文件作用域"]
    T14["函数原型作用域"]
