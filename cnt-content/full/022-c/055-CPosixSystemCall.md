...
- [文件 I/O 操作](/c/050-StdioFileIO)：理解流（FILE*）与缓冲的概念
...
处理函数内只能调用异步信号安全函数（write 等），只设置 `volatile sig_atomic_t` 标志；系统讲解见 [信号处理](/c/014-SignalHandling)。
...
- 与标准库层的分界：纯算法、字符串、内存用 C 标准库即可移植；一旦涉及进程/信号/套接字就离开可移植区，见 [C 标准库速查手册](/c/054-CStandardLibrary)。
