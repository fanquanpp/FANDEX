...
- [C 语言零基础起步](/c/010-CZeroBasisStart)：编译环境搭建、Hello World 逐行拆解、从源代码到可执行文件的五阶段流程。
- [程序结构与基本语法](/c/030-ProgramStructureBasicSyntax)：翻译单元、入口函数、语句与注释的最小闭环。
- [数据类型详解](/c/040-DataTypeDetailed)：基本类型、类型宽度与有符号性，是理解内存布局的地基。
...
1. 重读 [多文件编译](/c/310-MultiFileCompilation)，把项目从单文件重构为头文件加实现文件加 CMake 的标准布局。
2. 精读 [动态内存管理](/c/200-DynamicMemoryManagement) 第 5 章起的内容，理解 ptmalloc/jemalloc 的分配器原理。
3. 进入 [POSIX 线程](/c/370-POSIXThread) 与 [共享内存与信号量](/c/350-SharedMemorySemaphore)，从线程走向进程间协作。
4. 用 [Socket 网络编程](/c/390-SocketNetworkProgramming) 给音乐平台写一个最小的歌曲查询服务。
5. 通过 [C 项目示例：学生成绩系统](/c/580-CProjectExampleStudentGradeSystem) 完整演练一次模块化开发，再用 [Valgrind](/c/510-CValgrind) 做发布前的内存体检。
