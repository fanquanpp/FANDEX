...
OWASP Top 10:2025（2025-11 正式发布）的官方清单。与 2021 版相比，
「软件供应链失效」与「异常条件处理不当」为新晋类别，SSRF 与「过时组件」
不再单列（并入供应链失效）：
...
> 2021 版（A01 失效的访问控制 ... A10 SSRF）仍被大量存量资料引用，对照
> 阅读时注意版本差异；SSRF 测试在 2025 版中归入 A01 与 A03 的测试范围。
...
- 初学者要点：安全测试的入口是 OWASP Top 10 清单 + 一台 ZAP；注入类
  载荷（SQL 注入、XSS）可以直接固化为常规自动化用例；移动端兼容性优先
  覆盖「用户分布 top 设备 + 系统版本矩阵」，而不是追求全量真机。
- 进阶注意：安全测试的深水区在业务逻辑越权（IDOR），工具扫不出，必须
  写专项用例（A 用户调 B 用户资源应 403）；CI 中测试分层的顺序是「静态
  扫描 → 单元 → 集成 → E2E → 安全扫描」，越靠前越快越要强制；测试左移
  与质量内建的落点是「评审、门禁、自动化回归」三件事的制度化，而不是
  口号（门禁工程化见「CI/CD 测试门禁」）。
