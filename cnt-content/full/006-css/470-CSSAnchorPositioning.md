...
理解要点：`position-area` 选中区域后，浏览器会自动把弹层的 inset 与对齐方式调整为“填入该区域”，所以它一个属性就替代了“JS 算坐标 + 算居中”两步。它的选区思路与 Grid 的模板区域一致（`css/250-CSS3GridGridLayout`）。
...
锚点定位只管“贴在哪”，显示/隐藏与顶层层级交给 Popover API（`css/650-CSSNewFeatures` 中有该 API 的介绍）：
...
- 定位体系：`css/220-PositionDetailed`；
- 新特性总览：`css/650-CSSNewFeatures`；
- 层叠上下文与弹层层级：`css/230-StackingContext`；
- 特性检测：`css/460-FeatureQuery`；
- 可访问性样式：`css/500-AccessibleStyling`。
