...
在 Styles 面板里，被划掉的声明旁边通常有来源文件与行号。点击可跳到对应源码；悬停可看到覆盖它的规则。配合优先级速查（`css/180-CSSPriorityQuickStart`），就能判断是该改权重、改顺序，还是加更具体的类。
...
**讲解：** 布局“莫名其妙多了 20px”“两个盒子贴太近”这类问题，用盒模型图一眼就能看出是哪一层占的空间，再回到 `css/050-CSS3BoxModelDetailed` 查属性。
...
- 选择器匹配：`css/130-CSS3SelectorSystem`；
- 盒模型：`css/050-CSS3BoxModelDetailed`。
