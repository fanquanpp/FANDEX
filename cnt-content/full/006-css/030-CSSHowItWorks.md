...
本课只讲这条流程的“入门版”，让你知道样式为什么会生效、为什么不生效、以及为什么 `link` 要写在 `<head>` 里。深入版本见 `css/550-CriticalRenderPathOptimization`。
...
`css/550-CriticalRenderPathOptimization` 讲的是“如何让这条流程更快”：压缩 CSS、去掉阻塞、延迟非关键样式等。本课只要建立流程直觉：
...
- 关键渲染路径深入：`css/550-CriticalRenderPathOptimization`；
- 引入方式与渲染阻塞：`css/040-StyleSheetImportMethod`；
- 优先级计算：`css/170-PriorityCalculation`；
- HTML 结构基础：`html5/020-HTML5OverviewCoreFeature`。
