...
先看痛点。假设你想给弹层用上锚点定位（`css/051-CSSAnchorPositioning`），但不可能要求所有用户都用最新浏览器。直接写会怎样？
...
- 新特性总览与各特性的支持状态：`css/065-CSSNewFeatures`；
- 锚点定位（特性查询的典型应用对象）：`css/051-CSSAnchorPositioning`；
- 层叠层（比 @supports 更优雅的优先级治理）：`css/040-CascadeLayer`；
- 自定义属性与 @property：`css/036-CSSVariableCustomAttribute`；
- CSS 函数与 @supports 回退：`css/054-Function`。
