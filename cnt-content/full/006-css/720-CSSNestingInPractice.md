...
`css/480-CSSNativeNesting` 讲的是嵌套**语法**（`&` 怎么写、权重怎么算）；本篇讲的是嵌套**怎么在真实项目里用**：
...
超过三层的信号是“你在用 DOM 结构写样式”。解法是给深层节点独立类名（配合 BEM，见 `css/600-BEMNamingMethodology`），在顶层平铺。
...
原理与层的完整规则见 `css/450-CascadeLayer`。要点：进了层的样式永远低于无层样式，第三方样式最适合用 `@import ... layer(...)` 收进低优先级层。
...
决策依据很简单：**browserslist 里有不支持嵌套的浏览器，就把转译挂进构建链**；目标全绿时编译步骤可以省略。构建链集成细节见 `css/590-PostCSS`。
