...
3. **HTML 基础**：能写一个含 `<h1>` 与 `<p>` 标签的 `index.html`（HTML 入门见 `html5/020-HTML5OverviewCoreFeature`）。
...
如果以上任一项答不上来，建议回头重读对应小节；都能回答，就可以进入 `css/050-CSS3BoxModelDetailed`。
...
- 变量是运行时特性，可以被媒体查询与 JavaScript 动态覆盖（详见 `css/410-CSSVariableCustomAttribute`）。
...
- 选择器：`css/130-CSS3SelectorSystem` 系统掌握选择器；
- 盒模型：`css/050-CSS3BoxModelDetailed` 理解尺寸计算；
- 变量深入：`css/410-CSSVariableCustomAttribute` 作用域与动态主题；
- 响应式：`css/370-ResponsiveDesign` 与 `css/360-MediaQuery`；
- 工程化：`css/560-CSSArchitectureMethodology` 与 BEM 命名。
