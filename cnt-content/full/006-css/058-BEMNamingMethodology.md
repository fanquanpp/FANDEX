...
嵌套负责“组织”，BEM 负责“隔离”。关键约定：**类名写全，不用 `&` 拼接**（原生 CSS 的 `&__title` 是无效语法，见 `css/042-CSSNativeNesting` 第 7 节）：
...
更系统的组合规范见 `css/072-CSSNesting` 第 4 节。
...
CSS Modules 用构建期哈希实现机械隔离（`css/060-CSSModules`），BEM 用约定实现人文隔离。两者不冲突：很多团队在组件内用 CSS Modules、跨组件复用与全局形态用 BEM。选型对比：
...
- 架构方法论全景（SMACSS/ITCSS）：`css/044-CSSArchitectureMethodology`；
- 原生嵌套与 BEM 组合：`css/042-CSSNativeNesting`、`css/072-CSSNesting`；
- CSS Modules（构建期隔离）：`css/060-CSSModules`；
- 原子化 CSS（另一种组织范式）：`css/059-CSSAtomic`；
