...
没有这行，后面所有 rem/vw 的计算基准都是错的——它是适配的第一开关。完整属性说明见 `html5/036-ViewportConfigMobileFirst`。
...
`clamp(最小值, 首选值, 最大值)` 让一个属性在区间内**连续平滑**地随视口变化，不需要断点跳变（详见 `css/054-Function`）：
...
| 图片撑破容器 | 横向滚动 | `max-width: 100%`（详见 `css/067-ResponsiveImage`） |
| rem 方案全局等比 | 文字失真 | 字号单独用 clamp 封顶 |
| 桌面端回归遗漏 | 适配后桌面异常 | 移动优先写法 + 桌面冒烟测试 |

## 扩展学习

- 视口元标签详解：`html5/036-ViewportConfigMobileFirst`；
- 媒体查询：`css/032-MediaQuery`；
- 容器查询（组件级适配）：`css/033-ContainerQuery`；
- 响应式设计方法论：`css/034-ResponsiveDesign`；
- CSS 函数与单位：`css/054-Function`、`css/007-CSSValuesAndUnits`。
