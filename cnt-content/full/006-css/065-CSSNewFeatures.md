...
related:
  ...
  - 'css/030-CSSViewTransitions'
  - 'css/031-CSSScrollDrivenAnimations'
  - 'css/051-CSSAnchorPositioning'
  - 'css/041-FeatureQuery'
  - 'css/035-ModernColorSpace'
prerequisites:
...
- 写代码时用 `@supports`（见 `css/041-FeatureQuery`）做能力检测，而不是猜测版本号。
...
支持状态：规范仍在推进，2025 年起 Chromium 系提供早期实现，属**明确的新兴特性**——语法细节可能调整，生产项目建议观望或封装在构建层（见 `css/057-PostCSS`）。
...
限制与注意：跨文档过渡要求同源导航；捕获期间页面暂停渲染，回调里只做同步 DOM 修改；应尊重 `prefers-reduced-motion` 关闭过渡。完整机制、伪元素定制与降级方案见 `css/030-CSSViewTransitions`。
...
典型组合拳是 popover / dialog 的“打开淡入、关闭淡出且不闪断”，配合 `overlay` 属性还能让元素在退出动画期间留在顶层。完整示例见 `css/029-CSSAnimationTransition` 的“现代动画新特性”节。
...
| 特性 | 一句话定位 | 独立文档 |
| --- | --- | --- |
| 容器查询 `@container` | 组件按父容器尺寸响应，cqw/cqi 单位 | `css/033-ContainerQuery` |
| 样式查询 `style()` | 按容器自定义属性值选择样式 | `css/033-ContainerQuery` |
| 层叠层 `@layer` | 声明式控制优先级，收纳第三方样式 | `css/040-CascadeLayer` |
| 原生嵌套 + `&` | 无预处理器写嵌套选择器 | `css/042-CSSNativeNesting` |
| `:has()` | 按后代/兄弟状态选父级与前辈 | `css/008-CSS3SelectorSystem` |
| 逻辑属性 `margin-inline-*` | 书写模式无关的方向语义 | `css/037-LogicalProperty` |
| `color-mix()` / oklch / 相对颜色 / `light-dark()` | 现代颜色系统 | `css/035-ModernColorSpace` |
| 滚动驱动动画 `scroll()` / `view()` | 动画进度跟随滚动 | `css/031-CSSScrollDrivenAnimations` |
| 锚点定位 `position-area` / `anchor()` | 纯 CSS 弹层定位 | `css/051-CSSAnchorPositioning` |
| `@scope` | 选择器作用域限定（Don't/Scope 区间） | `css/071-ScopeAtRule` |
| `@property` | 注册带类型/初始值/可动画的自定义属性 | `css/036-CSSVariableCustomAttribute` |
| `text-wrap: balance` / `pretty` | 标题平衡折行、避免孤行 | `css/005-TextAndFontsBasics` |
| `accent-color` / `light-dark()` | 控件着色与明暗自适应 | `css/035-ModernColorSpace` |
...
4. **建设施而非散装检测**：把兼容性结论沉淀到团队 browserslist、构建配置（`css/057-PostCSS`）与 UI 组件库里，而不是每处样式各自 `@supports`。
...
- 容器查询与样式查询：`css/033-ContainerQuery`；
- 层叠层：`css/040-CascadeLayer`；
- 原生嵌套：`css/042-CSSNativeNesting`、`css/072-CSSNesting`；
- 视图过渡：`css/030-CSSViewTransitions`；
- 滚动驱动动画：`css/031-CSSScrollDrivenAnimations`；
- 锚点定位：`css/051-CSSAnchorPositioning`；
- 现代颜色：`css/035-ModernColorSpace`。
