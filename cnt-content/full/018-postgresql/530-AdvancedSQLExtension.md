...
-- 连续登录天数（间隙与岛屿经典解法）
-- 第一步: 对每个用户按日期排序，日期减去行号得到"分组键" grp。
-- 同一段连续日期的 grp 值相同（日期每天 +1，行号也 +1，相减恒定）。
-- 注意: 必须先 DISTINCT 去重（同一天多次登录只算一天），
--       不能像下面这样直接在外层加 GROUP BY——那会破坏窗口计算:
-- SELECT user_id, login_date,
--   login_date - (ROW_NUMBER() OVER (...))::int AS grp
-- FROM user_logins GROUP BY user_id, login_date;  -- 错误写法

-- 第二步: 按 (user_id, grp) 分组统计连续天数
