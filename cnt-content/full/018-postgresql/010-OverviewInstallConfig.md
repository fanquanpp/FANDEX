...
- PostgreSQL：18（2025-09 发布，当前最新小版本为 18.6，2026-08-13 发布）为主力稳定版；
  19 处于 Beta 阶段，预计 2026 年内正式发布。每个大版本支持 5 年（14 已临近 EOL，
  2026-11-12 结束支持），企业生产推荐 17 或 18。
...
### 1.3 PostgreSQL 17 新特性（2024-09）

```
- SQL/JSON 标准化: JSON_TABLE、JSON_QUERY 等
- MERGE 语句增强: 支持 RETURNING 子句
- 增量备份: pg_basebackup 支持增量备份（配合 pg_combinebackup 合并）
- VACUUM 改进: 死元组 TID 存储改用 radix tree，内存占用最多降低 20 倍
- 逻辑复制增强: 逻辑复制槽支持故障转移（sync_replication_slots）
- 性能提升: 流式 I/O 接口加速顺序扫描与 ANALYZE
```

### 1.4 PostgreSQL 18 新特性（2025-09）

```
- 异步 I/O: 新 io_method 参数（worker / io_uring），顺序扫描与 VACUUM 读取最高约 3 倍提升
- uuidv7(): 内置时间有序 UUID 生成函数，对索引更友好（uuidv4() 为 gen_random_uuid() 别名）
- 虚拟生成列: GENERATED ... VIRTUAL，查询时计算不占存储，且成为生成列的默认类型
- B-tree 跳跃扫描: 多列复合索引在省略前导列等值条件时也可使用索引
- 时态约束: 主键/唯一约束支持 WITHOUT OVERLAPS，外键支持 PERIOD
- pg_upgrade 升级后保留优化器统计信息，升级后不再需要重新 ANALYZE
- md5 密码认证被弃用，推荐迁移到 SCRAM-SHA-256；新增 OAuth 2.0 认证扩展点
```
...
# cert 认证即凭客户端证书完成身份验证，method 处不能再叠加 scram-sha-256
