...
related:
  ...
  - 'cybersecurity/030-CryptographyApplication'
...
| `Lax`    | 顶级导航的 GET 允许，POST/iframe 等禁止 |
| `None`   | 允许跨站携带（必须配合 `Secure`）    |

**推荐**：大多数场景使用 `SameSite=Lax`，兼顾安全与用户体验。

**时效要点**：Chrome 自 2020 年起对未显式设置 SameSite 的 Cookie 默认按 `Lax`
处理，且 `SameSite=None` 必须附带 `Secure` 属性；Firefox/Safari 也逐步跟进。
这意味着「新部署的站点」天然获得一层基础防护，但两点不能省：

- 兼容性 Cookie（未设置 SameSite 的旧系统）与部分内嵌场景仍在裸奔；
- `Lax` 挡不住顶级 GET 导航类请求，写操作绝不允许用 GET（见 2.1 节的反例）。
