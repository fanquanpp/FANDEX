...
RSA 与 ECC 的安全性都依赖经典计算机解不开的数学难题；**Shor 算法在大规模容错
量子计算机上可在多项式时间内破解整数分解与离散对数**——届时本章所有算法全部失效。
虽然这样的计算机尚未出现，但「先截获、后解密」（Harvest Now, Decrypt Later）
使长周期敏感数据今天就已暴露在风险中，迁移窗口以年计。

### 6.2 NIST 标准已落地

NIST 于 2024 年 8 月发布了首批后量子密码标准，均基于全新的数学难题
（格/哈希），与 RSA/ECC 问题族无关：

| 标准      | 算法     | 用途         | 数学基础       |
| :-------- | :------- | :----------- | :------------- |
| FIPS 203  | ML-KEM（原 Kyber） | 密钥封装（替代 ECDH/RSA 加密） | 模格 |
| FIPS 204  | ML-DSA（原 Dilithium） | 数字签名（替代 RSA/ECDSA） | 模格 |
| FIPS 205  | SLH-DSA（原 SPHINCS+） | 无状态哈希签名（保守备选） | 哈希 |

注意：传统哈希与对称加密不在「被攻破」名单内（见 014 第 6.1 节），
PQC 替换的对象是**公钥算法**。

### 6.3 当前的落地形态

- **混合密钥交换**：TLS 1.3 已支持把 X25519 与 ML-KEM 组合使用
  （如 X25519MLKEM768 组合），任一算法未被攻破即可保证机密性；
  Chrome/Firefox 等主流浏览器与主要 CDN 已默认启用混合组。
- **签名迁移更慢**：ML-DSA 的签名与公钥尺寸远大于 Ed25519（数十倍），
  影响证书链体积与握手，各 CA 与协议仍在分阶段推进。
- **工程建议**：新系统选库时确认支持 PQC/混合模式；长期保密数据从今天起
  规划密钥体系可替换性（crypto-agility），避免算法被「焊死」在协议里。
