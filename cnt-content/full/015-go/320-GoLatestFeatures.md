...
- [Go 泛型详解](/go/240-GenericDetailed)：泛型是本篇诸多特性的地基，类型参数与约束的写法需要先掌握。
- [Go 包管理详解](/go/310-PackageManagementDetailed)：go.mod、go.sum 与模块解析是理解工作区与工具链指令的前提。
- [Go 单元测试与基准测试](/go/360-UnitTestBenchmark)：升级项目的回归验证离不开测试工程化。
...
Go 每年发布两个大版本（2 月与 8 月），当前最新的两个版本会持续获得安全修复（截至本文更新时，最新版本为 2026 年 8 月发布的 Go 1.27，受支持版本为 1.27 与 1.26；实时信息以 [官方发布页](https://go.dev/doc/devel/release) 为准）。官方的 Go 1 兼容性承诺保证：符合规范的旧程序在新版本下仍能编译运行，这让升级的成本主要来自"语义随版本声明变化"与"依赖库适配"两处。近年各版本的代表性特性如下：
...
| 1.26 | 2026-02 | Green Tea GC 默认启用、new(expr)、泛型约束自引用、errors.AsType、go fix 重写 |
