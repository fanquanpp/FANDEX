...
- [unsafe 与指针](/go/300-UnsafePointer)：建议先完成前一篇的学习
...
### 1.3 Go 1.1（2013-05）：调度器重写与 Go 1.3（2014-06）：连续栈

Dmitry Vyukov 的可扩展调度器在 Go 1.1 落地：引入 P（逻辑处理器）、工作窃取与 P 本地运行队列，GMP 模型自此定型。Go 1.3 则把分段栈（segmented stack）替换为连续栈（contiguous stack）。此后 channel 的阻塞与唤醒通过 runtime 的 `gopark`、`goready` 与调度器深度集成：

- `gopark`：将当前 goroutine 置为 `Gwaiting` 状态，从 P 上摘下
- `goready`：将 goroutine 重新置为 `Grunnable`，放回运行队列等待调度
...
### 1.5 Go 1.18（2022-03）：泛型让 channel 算法可参数化

channel 类型自诞生起就是参数化类型（`chan T` 由编译器保证元素类型安全）；Go 1.18 的泛型进一步让"围绕 channel 的算法"可以写成类型安全的泛型函数，例如合并任意元素类型的多个 channel：
...
### 1.6 Go 1.22 / 1.23（2024）：range over function 迭代器

Go 1.22 以 `GOEXPERIMENT=rangefunc` 引入 range over function 实验特性；Go 1.23 起正式稳定，任何 `func(yield func(V) bool)` 形态的迭代器函数都可以被 `for range` 直接消费，标准库新增 `iter` 包提供 `Seq`/`Seq2` 类型与 `iter.Pull`。该特性针对迭代器协议而非 channel 本身——channel 的遍历仍由原有的"直到 close"语义承担，但两者现在可以互相转换（`slices.Collect`、`slices.Values` 等）。同期的运行时改动包括优化 channel 内部锁的实现路径，减少不必要的 atomic 操作。
...
timeline
    ...
    2022: Go 1.18 泛型让 channel 元素类型抽象可复用
    2024: Go 1.22 range over func 实验，Go 1.23 迭代器协议稳定
```
