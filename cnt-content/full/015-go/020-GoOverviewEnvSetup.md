...
- Go：1.27.x 为当前稳定系列（1.27.0 于 2026-08 发布）；Go 每年发布两个大版本，官方仅支持最近两个大版本（当前为 1.27 与 1.26），存量 1.25/1.24 应尽快升级。实时版本以 [官方下载页](https://go.dev/dl/) 为准。
...
| Go 1.24 | 2025.02 | 泛型类型别名、弱指针（weak）、os.Root、Swiss Table map 运行时       |
| Go 1.25 | 2025.08 | 容器感知 GOMAXPROCS、`sync.WaitGroup.Go`、testing/synctest 稳定    |
| Go 1.26 | 2026.02 | Green Tea GC 默认启用、`new(expr)`、errors.AsType                   |
| Go 1.27 | 2026.08 | 当前最新稳定线（实时特性以 [发布说明](https://go.dev/doc/devel/release) 为准） |
...
**Go 1.25 — sync.WaitGroup.Go 与容器感知 GOMAXPROCS**：

```go
var wg sync.WaitGroup
wg.Go(func() { // 1.25 起：Add/Done 与 go 语句合并为一步
    processTask()
})
wg.Wait()
```

同时，1.25 的运行时在容器内会按 cgroup CPU 配额自动设定 `GOMAXPROCS` 默认值，此前需手动设置或借助 automaxprocs 等第三方库。
