...
- [垃圾回收与 GC 调优](/go/290-GCAndTuning)：建议先完成前一篇的学习
...
### 1.9 Go 1.22 / 1.23（2024）：range over function 与 iter 包

Go 1.22 以 `GOEXPERIMENT=rangefunc` 预览 range over function，Go 1.23 正式稳定：返回 `func(yield func(V) bool)` 的函数可被 `for range` 直接消费，标准库 `iter` 包提供 `Seq`/`Seq2` 类型别名。注意它与"对返回切片的普通函数做 range"是两回事——迭代器的元素是惰性逐个产出的：

```go
// 泛型迭代器：把任意切片变换为惰性序列（Go 1.23+）
func Map[E, U any](s []E, f func(E) U) iter.Seq[U] {
    return func(yield func(U) bool) {
        for _, v := range s {
            if !yield(f(v)) {
                return // 消费方提前 break 时停止产出
            }
        }
    }
}

// range over function：for range 直接消费迭代器
for v := range Map([]int{1, 2, 3}, func(x int) int { return x * 2 }) {
    fmt.Println(v) // 依次输出 2 4 6
}
```
...
- **proposal: range over function** (Go 1.22 以 GOEXPERIMENT 预览，Go 1.23 稳定)
- **proposal: generic type aliases** (Issue #46477，Go 1.24 稳定)
