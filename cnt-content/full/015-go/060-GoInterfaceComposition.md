...
1. 理解接口的隐式实现与"方法集"规则，能判断值类型与指针类型各满足哪些接口。
2. 掌握空接口 `any`、类型断言与类型开关的用法与边界。
3. 说清"接口为 nil"与"接口内指针为 nil"的区别，避开经典陷阱。
4. 会用接口组合与 io.Reader/Writer 抽象构建可插拔的代码结构。
...
func (h *hits) Inc()     { h.n++ } // 指针接收者
func (h hits) Value() int { return h.n }

var c Counter = &hits{}   // 正确：*hits 的方法集包含 Inc 与 Value
// var c Counter = hits{} // 编译错误：hits 的方法集只有 Value，未实现 Inc
```

```text
# 若取消注释，编译报错：
cannot use hits{} (value of type hits) as Counter value:
hits does not implement Counter (method Inc has pointer receiver)
```

记忆方法：值接收者语义上是"副本调用"，编译器不允许通过接口拿到可变的副本；指针接收者意味着方法可能修改状态，因此只有指针能进接口。**实践中若拿不准，结构体方法统一用指针接收者**是社区常见默认，可避免这类方法集劈叉。
