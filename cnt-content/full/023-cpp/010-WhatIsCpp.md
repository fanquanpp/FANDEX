...
C++ 是编译型语言，需要编译器（Windows 推荐 VS Code + MinGW，配置见 [C 语言零基础起步](/c/010-CZeroBasisStart) 的环境章节，两者共用工具链）。保存 `hello.cpp` 后：
...
进入 [C++ 概述与现代标准](/cpp/020-CppOverviewAndModernStandard) 开始主线；学到智能指针与 STL 章节时，你会真正体会到"现代 C++"与教科书上古老形象的巨大差异。
