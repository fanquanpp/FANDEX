...
像素是离散的,而 SVG 坐标是连续的。把设备像素看作单位方格:像素行 $k$ 覆盖 $[k, k+1)$,其**边界**在整数 + 0.5 处。一条 `stroke-width="1"` 的水平线,描边以路径为中心向两侧各延伸 0.5:

- 路径放在**整数坐标**(如 $y = 10$):描边覆盖 $[9.5, 10.5]$,横跨像素行 9 与 10 各一半,抗锯齿给两行各分配约 50% 的颜色,1px 线显示为约 2px 的模糊灰线
- 路径放在**半像素坐标**(如 $y = 10.5$):描边恰好覆盖 $[10, 11)$,与像素行 10 完全重合,边缘锐利

$$
I_{\text{display}}(p) = \int_{\text{pixel}(p)} I_{\text{ideal}}(x) \, dx
$$

其中 $I_{\text{ideal}}$ 为理想连续图像,$I_{\text{display}}$ 为离散像素值。这就是"0.5 偏移技巧"的数学原理:**1px 描边对齐半像素坐标、填充边缘对齐整数坐标**。注意该技巧仅在 1:1 渲染时成立;viewBox 有缩放或设备像素比(DPR)非 1 时,最终是否锐利取决于缩放后的实际落点。
...
排查 viewBox 问题时,最直接的办法是给 `<svg>` 加边框,让视口边界可见:
...
再配合 DevTools 的 Elements 面板查看计算后的 viewport 尺寸,就能快速判断"内容太大被裁"还是"视口比例不匹配留白"。
