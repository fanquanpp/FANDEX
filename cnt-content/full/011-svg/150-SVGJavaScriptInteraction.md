...
> 前置知识：CSS 样式化的优先级规则（《SVG CSS 样式化》，014-SVGCSSStyling）、基本的 DOM 与事件模型。
>
> 学习目标：掌握 SVG DOM 与 HTML DOM 的差异（className、createElementNS、几何属性读写）；能把鼠标事件坐标正确换算为用户坐标（拖拽）；会做数据驱动的 enter/update/exit 渲染；了解 SVG 与 Canvas/位图互转的路径与限制。
...
拖拽的核心难点是**坐标换算**：`MouseEvent` 给的是屏幕坐标（clientX/Y），而 SVG 属性用的是用户坐标。viewBox 有缩放或平移时两者不相等，必须经 `getScreenCTM().inverse()` 转换：
...
svg.addEventListener('pointermove', (e) => {
  ...
  const p = toSVGPoint(e.clientX, e.clientY);
  circle.setAttribute('cx', p.x + offset.x);
  circle.setAttribute('cy', p.y + offset.y);
});
...
> 用 Pointer Events（pointerdown/move/up）替代 mouse 系列可同时兼容触屏；`setPointerCapture` 保证指针移出元素后事件不丢。
...
- SVG 里若引用了**外部资源**（外链图片、外链字体），转 Canvas 时这些部分加载不出来，甚至导致画布被判定污染而无法导出——导出前把外部资源内联为 data URL。
- `canvas.width` 要按 SVG 的实际渲染尺寸（含 devicePixelRatio）设置，否则导出图会模糊。
...
// base64 形式（unescape 已废弃但仍是兼容性最好的写法）
const dataUrl = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(svgString)));

// 也可用 URL 编码形式，体积更小且可直接肉眼检查
const dataUrl2 = 'data:image/svg+xml,' + encodeURIComponent(svgString);
```
...
- 创建 SVG 节点必须 `createElementNS('http://www.w3.org/2000/svg', tag)`，用 `createElement` 建出来的 rect 不会渲染；改类名用 `classList`，别直接赋值 `className`。
- 鼠标事件里 `e.clientX` 是屏幕像素，SVG 属性是用户坐标，二者只在 1:1 无偏移时相等；换算统一走 `getScreenCTM().inverse()`。
- 绘制类动画的两大工具：`getTotalLength()` / `getPointAtLength()`（沿路径定位），配合 dashoffset 做描线。

进阶注意：

- 频繁 `setAttribute('x'/'y')` 会走布局，动画尽量用 `transform`；高频更新考虑把整块图形移到 `<canvas>`（见《SVG 响应式与性能》的选型表）。
- 序列化导出前检查外部资源：外链图片/字体不仅转 Canvas 会失败，`<img>` 引用与 data URL 场景同样会被安全策略拦下。
- WAAPI（`el.animate`）是 JS 驱动动画的现行标准，性能接近 CSS 动画且可暂停/反转/读取 currentTime，适合与交互状态联动。
