...
`dominant-baseline` 定义文本垂直对齐所用的基线。设基线 $y_b$ 相对于 `y` 属性的偏移为 $\Delta y$(SVG 的 y 向下为正,负值表示向上移动),记 $x_h$ 为 x-height(小写字母高度),$a$ 为字体升部(ascent),$d$ 为降部(descent)深度:

| 值 | 基线类型 | $\Delta y$(近似) |
| -- | -------- | ---------- |
| `alphabetic` | 字母基线(默认) | 0 |
| `middle` | 基线到 x-height 的中点 | $-x_h/2$ |
| `central` | em 盒(升部到降部)垂直中点 | $-(a-d)/2$ |
| `hanging` | 悬挂线(天城文等) | $-a_{sc}$ |
| `text-top`(即 `text-before-edge`) | 文本顶部 | $-a_{sc}$ |
| `text-bottom`(即 `text-after-edge`) | 文本底部 | $+d_{esc}$ |

这些基线对应不同书写系统的传统参考线;$a_{sc}$ / $d_{esc}$ 为对应书写系统的升部/降部。注意 `middle` 与 `central` 是两条不同的基线,不同字体度量下差距可达数像素,且浏览器对 `central` 的实现历来有差异——垂直居中文字时优先实测。
...
`writing-mode="tb"`(top-to-bottom)让文字垂直排列,适合中日韩排版。注意 `tb` / `rl` / `lr` 是 SVG 1.1 的遗留取值;SVG 2 与 CSS Text 对齐,标准值为 `horizontal-tb` / `vertical-rl` / `vertical-lr`(见 2.6 节)。遗留值在主流浏览器中仍被映射支持,但新代码建议统一用下面的 CSS 标准值。
...
> 竖排文本的浏览器实现差异比横排大(尤其中亚/蒙古文等复杂方向),关键场景务必在目标浏览器实测。
