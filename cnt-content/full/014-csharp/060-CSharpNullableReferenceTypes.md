...
- [C# 基础语法](/csharp/030-CSharpBasicSyntax)：变量、方法与类的基本写法，是理解注解语法的前提。
- [值类型与引用类型](/csharp/050-ValueTypeReferenceType)：`null` 只会出现在引用类型上，理解这一点才能把握 NRT 的作用边界。
- [C# 泛型与集合](/csharp/070-CGenericCollection)：本篇会涉及泛型约束 `where T : notnull` 与 `T?` 在泛型中的特殊含义。
