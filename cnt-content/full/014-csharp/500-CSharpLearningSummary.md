...
- [C# 是什么：.NET 世界的通用语言](/csharp/010-WhatIsCSharp)：理解 C# 的定位、IL 与 .NET Runtime 的运行模型。
- [C# 面向对象编程](/csharp/040-CSharpOOP)：类、属性、继承与多态是 C# 语法体系的主干。
- [C# 泛型与集合](/csharp/070-CGenericCollection)：泛型与集合是 LINQ、EF Core 与几乎所有工程代码的地基。
...
1. **深入异步**：[C# 异步编程详解](/csharp/090-AsyncProgrammingDetailed) 与 [async/await 状态机](/csharp/100-AsyncAwaitStateMachine)，从"会用"进阶到"理解编译器生成的状态机"。
2. **深挖 LINQ**：[LINQ 深度剖析](/csharp/120-LINQDeep) 与 [延迟执行与立即执行](/csharp/130-LINQDeferredImmediate)，掌握表达式树与查询翻译的边界。
3. **打通框架链路**：[C# 与 EF Core](/csharp/280-CSharpEFCore)、[C# 依赖注入](/csharp/260-CSharpDependencyInjection) 与 [ASP.NET Core 中间件管道](/csharp/310-AspNetCoreMiddlewarePipeline)，构建完整的服务端知识体系。
4. **关注运行时性能**：[GC 分代](/csharp/390-GCGeneration)、[Span 与 Memory](/csharp/230-SpanMemory) 与 [.NET 性能与基准测试](/csharp/400-DotnetPerformanceBenchmarking)，学会用数据驱动优化。
5. **拓展平台方向**：游戏方向进入 [C# 与 Unity 游戏开发](/csharp/380-CSharpUnityGameDev)，客户端方向了解 [C# 与 MAUI](/csharp/330-CSharpMAUI) 与 [C# 与 Blazor](/csharp/320-CSharpBlazor)。
