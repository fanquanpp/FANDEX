...
- [Span 与 Memory](/csharp/230-SpanMemory)：零分配字符串与缓冲区操作是本篇优化清单的主角。
- [GC 分代](/csharp/390-GCGeneration)：解读分配数据与 Gen0/1/2 回收次数需要分代回收的背景知识。
- [C# 测试工程化](/csharp/340-CSharpTestEngineering)：基准项目与测试项目同构，都要求可重复、可自动化。
