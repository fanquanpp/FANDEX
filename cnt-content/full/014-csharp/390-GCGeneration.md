...
related:
  ...
  - 'csharp/180-RecordTypeImmutability'
prerequisites:
...
- [依赖注入生命周期](/csharp/270-DILifecycle)：建议先完成前一篇的学习
...
.NET 1.0 引入了**三代模型**（Gen 0/1/2）与 **LOH**（Large Object Heap，≥85,000 字节的对象直接进 LOH）。设计由 Patrick Dussud 主导，参考了 Ungar 分代、Boehm-Demers-Weiser 标记清除与 Java HotSpot 的早期设计。
...
flowchart TD
    ...
    MH --> LOH[LOH &gt;=85,000B objects<br/>collected with Gen 2]
