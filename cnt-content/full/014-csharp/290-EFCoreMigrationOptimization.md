...
related:
  ...
  - 'csharp/360-ReflectionAndFeatureApplication'
...
- [反射与特性应用](/csharp/360-ReflectionAndFeatureApplication)：建议先完成前一篇的学习
...
- **`LeftJoin`/`RightJoin` 运算符**：直接翻译 .NET 10 新增的一等 LINQ 连接运算符，替代 `SelectMany + GroupJoin + DefaultIfEmpty` 组合。
- **复杂类型增强**：支持可选（可空）复杂类型、struct 复杂类型与映射到 JSON 列（配合 SQL Server 2025 原生 `json` 类型）。
- **`ExecuteUpdateAsync` 支持普通 lambda**：不再强制表达式树，可条件化动态构建更新列。
- **向量搜索正式化**：SQL Server 2025 / Azure SQL 的 `vector` 类型与 `VECTOR_DISTANCE` 一等支持，Cosmos DB 全文与混合检索增强。
