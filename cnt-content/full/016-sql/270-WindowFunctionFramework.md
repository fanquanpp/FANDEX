...
> **关键区别**：`ROW_NUMBER`/`RANK`/`DENSE_RANK`/`NTILE`/`LAG`/`LEAD` 这类函数
> **根本不使用窗口帧**（写 frame 子句会被忽略或报错）；真正受帧影响的是聚合函数与
> `FIRST_VALUE`/`LAST_VALUE`/`NTH_VALUE`，它们的默认帧取决于是否指定 ORDER BY。
...
| 场景                                              | 默认帧                                                     |
| ------------------------------------------------- | ---------------------------------------------------------- |
| 排名函数（ROW_NUMBER/RANK 等）、LAG/LEAD          | 不使用帧，帧子句对它们无效                                 |
| 有 ORDER BY，聚合/取值函数（FIRST_VALUE 等）      | `RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`        |
| 无 ORDER BY，聚合/取值函数                        | 整个分区（`UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`）  |
