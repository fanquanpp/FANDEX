...
窗口函数（Window Functions）随 SQL 标准的 OLAP 扩展诞生（SQL:1999 附加修订引入，SQL:2003 正式并入主标准），它能在不折叠行的情况下执行跨行计算。与聚合函数不同，窗口函数不会将结果分组为单行，而是为每一行返回一个基于"窗口"的计算值。如今三家主流数据库均已支持：PostgreSQL 8.4+、MySQL 8.0+、SQLite 3.25+，可以放心在业务中使用。
