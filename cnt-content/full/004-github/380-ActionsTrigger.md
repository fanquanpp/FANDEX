...
安全实践在持续收紧：官方 `actions/checkout` 自 v7 起，在 `pull_request_target` / `workflow_run` 事件中**默认拒绝检出 fork PR 的代码**（防止"用 secrets 跑不可信代码"的经典注入路径）；确需旧行为必须显式声明 `allow-unsafe-pr-checkout: true`。规范的替代方案是拆成两个 job——`pull_request_target` 的受信 job 只负责"提权动作"（如加标签、发评论），不可信代码的构建/测试交给 `pull_request` 事件的上传制品（artifact）接力。
...
| 常见错误 | 报错/现象 | 原因 | 解决办法 |
| --- | --- | --- | --- |
| `branches` 与 `branches-ignore` 同时使用 | `Invalid workflow file` 校验失败 | 正负面清单互斥 | 只保留其中一个，改用 `!` 否定模式 |
