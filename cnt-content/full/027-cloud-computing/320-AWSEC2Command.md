...
**概念 30 秒**：一台 EC2 = AMI 镜像 + 实例规格 + 网络（子网/安全组）+
密钥对。三个生命周期动词要分清：`stop`（停机不收费计算费，公有 IP 会
变）、`terminate`（销毁，默认同时删除根卷）、`reboot`（原地重启）。
实例选型与计费模型见 `270-AWSCore`。
...
**基本写法：开放 SSH 端口**
`aws ec2 authorize-security-group-ingress --group-id <安全组ID> --protocol tcp --port 22 --cidr <IP 段>`
```bash
...
- 初学者要点：查实例用 `describe-instances --filters` + `--query`；
  stop 后公有 IP 会变，需要固定公网地址就挂弹性 IP；terminate 默认删
  根卷但保留额外数据卷；私钥只在 `create-key-pair` 时下发一次。
- 进阶注意：闲置的弹性 IP 计费（未绑定实例时），清理实例时连 EIP 一起
  释放；安全组 SSH 不对全网开放，优先用 SSM Session Manager 免密钥
  登录；EBS 卷与快照都是持续计费项，`describe-volumes` 定期审计；
  创建卷时可用区必须与目标实例一致，否则无法挂载。
