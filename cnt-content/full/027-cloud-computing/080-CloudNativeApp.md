...
- **初学者要点**：云原生的四块基石是容器、微服务、声明式 API 与不可变基础设施；
  12-Factor 里最先落地的是配置外置、无状态进程、日志事件流三条；镜像优化记住
  "多阶段构建 + 精简基础镜像 + 非 root"三板斧。
- **进阶注意**：CPU limit 引发的 Throttle 比资源不足更隐蔽（P99 毛刺的经典成因，
  可评估去掉 CPU limit 只保留 request）；QoS 等级由 requests/limits 的组合隐式决定，
  核心服务建议 Guaranteed；HPA 伸缩算法基于比率而非绝对值，指标抖动靠
  stabilizationWindow 平抑。
