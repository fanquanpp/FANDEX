...
- 初学者要点：一切资源属于资源组，删除组等于删全部资源（清理利器，
  也是最大风险）；`az vm create` 一条命令连带创建 NIC/公网 IP/NSG；
  停机省计算费必须用 `deallocate`，仅 `stop` 仍计费；SSH 公网 IP 用
  `az vm show -d --query publicIps -o tsv` 取。
- 进阶注意：`run-command invoke` 免 SSH 远程执行命令，适合批量运维与
  排障；磁盘（Managed Disk）独立于 VM 生命周期，删除 VM 后按需清理，
  否则持续计费；生产 VM 建议配置自动关闭（DevTest 计划或 Azure
  Automation）防止开发机整夜空转。
