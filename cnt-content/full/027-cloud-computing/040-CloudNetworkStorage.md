...
# 安装 Docker（Amazon Linux 2 用 amazon-linux-extras；AL2023 改用 dnf install docker）
...
# 创建存储桶
# 注意：us-east-1 是默认区域，不能传 LocationConstraint，否则报错
s3.create_bucket(Bucket='my-app-bucket')

# 其他区域必须显式指定位置约束
# s3.create_bucket(
#     Bucket='my-app-bucket-eu',
...
- **初学者要点**：VPC 是你在云上的"自留地"，公有子网放入口（LB/NAT）、私有子网放
  应用与数据库；安全组是有状态的实例防火墙，NACL 是无状态的子网防火墙；存储三兄弟
  ——块存储挂盘、对象存储放文件、文件存储做共享目录。
- **进阶注意**：NAT 网关按小时 + 按流量双计费，大流量场景用 VPC Gateway Endpoint
  直连 S3 可省下这笔钱；对象存储按"最短存储时间 + 检索费"设计陷阱，频繁回迁的
  冷数据反而更贵；CDN 缓存命中率是第一个要看的指标。
