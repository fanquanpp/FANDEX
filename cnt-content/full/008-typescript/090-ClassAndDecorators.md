...
> 本节及后续"装饰器工厂""类/方法/属性装饰器实战"速查均为**实验性装饰器**签名（`experimentalDecorators: true`）。标准装饰器（TS 5.0+ 默认）使用 `(value, context)` 两参数签名且不含参数装饰器，详见 `typescript/280-DecoratorStandardImpl`。
