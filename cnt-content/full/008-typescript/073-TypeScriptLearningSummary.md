...
## 2026-09 增补：版本演进与新增专篇

1. `050-TypeScript5xNewFeatures`：5.0-5.9 逐版本特性演进（const 类型参数、标准装饰器、using、推断类型谓词、import defer 等）；
2. `068-TypeScript6And7CompilerEvolution`：6.0 默认值翻转与弃用清单、7.0 Go 原生编译器（tsgo）完成度与迁移决策；
3. `069-JSDocTypeDevelopment`：不写 .ts 也有完整类型检查的 JSDoc 路线。

### 同主题双篇的读法指引

本模块存在多组"基础篇 + 进阶篇"组合，按以下顺序读，不要只读其一：

| 主题 | 先读 | 再读 |
| --- | --- | --- |
| 类型体操 | `028-TypeGymnasticsPracticalPatterns`（实用模式） | `058-TypeGymnastics`（通用方法论） |
| infer 与条件类型 | `036-InferTypeDeepDive`（infer 专精） | `054-ConditionalTypeInfer`（组合运用） |
| 映射类型 | `021-MappedTypeAdvanced`（进阶基础） | `056-MappedTypeKeyRemap`（键重映射） |
| 装饰器 | `017-ClassDecorator`（类起步）→ `023-DecoratorDetailed`（详解） | `061-DecoratorStandardImpl`（标准实现） |
| 模块解析 | `026-ModuleResolutionModernToolchains`（现代工具链） | `039-TypeScriptTypeDeclarationModuleResolution`（声明配套） |
| 声明文件 | `024-DeclarationFileWriting`（编写） | `059-ModuleDeclarationGlobalAugmentation`（全局增强） |
...
3. 学习 [TypeSafe API Client](/typescript/041-TypeSafeAPIClient) 与 [运行时 Schema 校验](/typescript/072-RuntimeSchemaValidation)，把类型安全推进到网络边界。
4. 按 [TypeScript 迁移实践](/typescript/053-TypeScriptMigrationPractice) 与 [项目引用与 Monorepo](/typescript/071-ProjectReferencesMonorepo) 把存量 JS 仓库分阶段迁入严格模式。
5. 以 [项目实战：类型安全 API 客户端](/typescript/062-TypeScriptProjectExampleTypeSafeAPIClient) 与 [TS7 编译器指南](/typescript/068-TypeScript6And7CompilerEvolution) 收官，站在工具链演进的视角审视自己的工程配置。
