...
- [为什么需要 TypeScript：从 JavaScript 的烦恼说起](/typescript/010-WhyTypeScript)：JS 的类型陷阱与 TS 的价值定位。
- [TS 基础：变量与类型](/typescript/040-TSBasicsVariablesAndTypes)：基础标注语法与推断规则。
- [JavaScript 是什么](/javascript/010-WhatIsJavaScript)：TS 是 JS 的超集，语言基础决定类型学习的上限。
...
1. `670-TypeScript5xNewFeatures`：5.0-5.9 逐版本特性演进（const 类型参数、标准装饰器、using、推断类型谓词、import defer 等）；
2. `680-TypeScript6And7CompilerEvolution`：6.0 默认值翻转与弃用清单、7.0 Go 原生编译器（tsgo）完成度与迁移决策；
3. `400-JSDocTypeDevelopment`：不写 .ts 也有完整类型检查的 JSDoc 路线。
