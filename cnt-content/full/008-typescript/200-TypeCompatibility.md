...
泛型参数相同的两个实例化类型按普通规则判断；泛型参数不同时，还要看类型参数本身是否兼容（例如 `Box<Dog>` 能否赋给 `Box<Animal>` 取决于 `Box<T>` 中 `T` 出现的位置，详见 `260-CovarianceContravariance`）。
...
- `260-CovarianceContravariance`：型变理论的完整展开与工程案例；
- `120-IntersectionTypeMerge`：交叉类型合并时的属性冲突（冲突属性变成 never）；
- `110-LiteralUnionTypes`：类型收窄如何让联合类型"变成"单类型；
- `160-TypeInferenceDeepDive`：推断与兼容性如何配合工作。
