...
- [JavaScript 是什么：你的第一门编程语言](/javascript/010-WhatIsJavaScript)：语言定位、运行环境与第一行代码。
- [变量与数据类型](/javascript/040-VariableDataType)：let/const/var、原始类型与引用类型的分野。
- [程序结构与基本语法](/javascript/030-ProgramStructureBasicSyntax)：语句、块与注释的最小语法闭环。
...
本模块在 2026-09 完成一次知识点补齐，回看时优先补读四篇：

1. `590-ES2023To2026NewFeatures`：按年份串讲 ES2023-ES2025 全部定稿特性与 ES2026 展望；
2. `620-ExplicitResourceManagement`：`using`/`await using` 确定性资源释放（Stage 4，ES2026 核心）；
3. `610-TemporalJavaScriptAPI`：Temporal 已升格为 ES2026 标准，状态与旧文不同；
4. `630-ImmutableDataStructures`：Records & Tuples 提案已撤回，深不可变的现代替代方案；
5. `650-IntlApi`：内置国际化（ECMA-402）；`560-PackageJsonEngineeringDeepDive`：package.json 工程契约与 lint/format 工具链。

## 后续学习路径

1. 精读 [事件循环详解](/javascript/300-EventLoopDetailed) 与 [Promise 构造函数深度解析](/javascript/260-PromiseConstructorDeepDive)，把执行模型焊死在脑子里。
2. 进入 [内存管理与垃圾回收](/javascript/350-MemoryManagementAndGarbageCollection) 与 [内存泄漏排查](/javascript/370-MemoryLeakTroubleshoot)，掌握 DevTools 内存面板的完整用法。
3. 学习 [Fetch API 与 Web Streams](/javascript/450-FetchApiWebStreams) 与 [IndexedDB](/javascript/470-IndexedDBADatabaseInYourBrowser)，给音乐平台补上离线缓存能力。
4. 用 [Web Workers 多线程](/javascript/670-WebWorkersMultithreading) 把歌词解析搬出主线程，再以 [Service Worker 与 PWA](/javascript/680-ServiceWorkerPWA) 实现整站离线可用。
5. 以 [JavaScript 项目实战：Todo 应用](/javascript/700-JavaScriptProjectExampleTodoApp) 与 [调试与性能优化](/javascript/500-DebugPerformanceOptimization) 收官，完整走一遍开发、性能度量与上线流程；随后即可进入 TypeScript 模块。
