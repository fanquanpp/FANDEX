...
- [调试与性能优化](/javascript/500-DebugPerformanceOptimization)：知道 Performance 面板在哪里即可。
...
- 优化抓手：拆分长任务（`scheduler.yield()` / `setTimeout` 让出主线程）、复杂计算交给 Web Worker（见 `javascript/670-WebWorkersMultithreading`）、高频事件节流、优先更新"用户能看到的反馈"再处理其余逻辑。
