...
2. **Node.js**：把 V8 引擎从浏览器里"拆出来"装到操作系统上，让 JS 脱离浏览器独立运行——写后端、跑构建工具都靠它（安装见 [Node.js 安装配置](/javascript/052-NodeJsInstall)）。
...
3. **改了代码没生效**：控制台里的代码是临时的，刷新页面就没了；真实项目代码要写进 `.js` 文件（这一步在下一篇与 [Node.js 安装](/javascript/052-NodeJsInstall) 之后展开）。
