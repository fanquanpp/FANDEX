...
- [JavaScript 是什么](/javascript/001-WhatIsJavaScript)：知道怎么打开浏览器控制台即可。
...
> `time/timeEnd` 适合开发期的粗粒度对比（"这个写法是不是更慢"）；需要精确、可报告的数据请用 `performance.now()` 或 Performance API（见 [前端性能指标](/javascript/051-CoreWebVitalsAndPerformanceMetrics)）。
