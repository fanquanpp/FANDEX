...
- [正则表达式](/javascript/011-Regex)：建议先完成前一篇的学习
...
### 5.5 与重复命名捕获组（ES2025）的关系

ES2018 规定同一正则内组名必须唯一，两个分支无法复用 `(?<year>...)` 这样的名字，只能各起别名再在代码里归一。ES2025 的重复命名捕获组提案放开了这一限制：

```javascript
// ES2025：两个分支复用同一组名，匹配结果取"实际参与匹配的那个分支"的值
...
要点：组名重复时每个分支各自声明，运行时只有**匹配成功分支**的组会出现在 `groups` 里；`groups` 对象上同名只保留一个值。该特性向后兼容：旧正则（组名唯一）行为不变。
...
> 注：ES2025 的重复命名捕获组特性放开了这一限制，但仅允许**不同分支（`|` 两侧）**复用组名；
> 上例这种同一序列里紧跟两个同名组在任何版本下都非法。
