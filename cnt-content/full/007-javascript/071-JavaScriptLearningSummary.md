...
1. `061-ES2023To2026NewFeatures`：按年份串讲 ES2023-ES2025 全部定稿特性与 ES2026 展望；
2. `064-ExplicitResourceManagement`：`using`/`await using` 确定性资源释放（Stage 4，ES2026 核心）；
3. `063-TemporalJavaScriptAPI`：Temporal 已升格为 ES2026 标准，状态与旧文不同；
4. `065-ImmutableDataStructures`：Records & Tuples 提案已撤回，深不可变的现代替代方案；
5. `067-IntlApi`：内置国际化（ECMA-402）；`057-PackageJsonEngineeringDeepDive`：package.json 工程契约与 lint/format 工具链。
...
4. 用 [Web Workers 多线程](/javascript/069-WebWorkersMultithreading) 把歌词解析搬出主线程，再以 [Service Worker 与 PWA](/javascript/070-ServiceWorkerPWA) 实现整站离线可用。
5. 以 [JavaScript 项目实战：Todo 应用](/javascript/060-JavaScriptProjectExampleTodoApp) 与 [调试与性能优化](/javascript/050-DebugPerformanceOptimization) 收官，完整走一遍开发、性能度量与上线流程；随后即可进入 TypeScript 模块。
