...
1. 直接分发 .c 源码意味着用户机器必须有编译器；正确做法是构建各平台的 wheel（`.so`/`.pyd` 已编好），发布流程见 [包发布](/python/740-PackagePublish) 与 [打包演进](/python/730-PythonPackagingEvolution)。
...
3. **C 扩展长时间持有 GIL**：纯计算段不释放 GIL，多线程调用的收益归零（见 [GIL 与自由线程](/python/650-GILAndFreeThreading)）。计算前后用 `Py_BEGIN_ALLOW_THREADS`/`Py_END_ALLOW_THREADS` 包住不触碰 Python 对象的段落。
