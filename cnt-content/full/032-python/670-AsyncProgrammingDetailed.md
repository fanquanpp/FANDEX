...
- [元类与单例模式](/python/600-MetaclassSingleton)：建议先完成前一篇的学习
...
asyncio.get_running_loop().set_debug(True)  # 在协程内调用；启动期可用 -X dev 或 PYTHONASYNCIODEBUG=1
```
...
# 正确：子线程里用 asyncio.run 自建自管循环（一次性任务首选）
def good():
    asyncio.run(coro())

# 正确：需要自管循环生命周期时，显式创建并关闭
def good_manual():
...
Django 3.1+ 支持异步视图，4.1+ 提供异步 ORM 接口，4.2+ 适配 psycopg3 异步驱动：
