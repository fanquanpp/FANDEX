...
3. 装饰器与代理函数转发参数时才写 `def wrapper(*args, **kwargs): return f(*args, **kwargs)`，并配合 `functools.wraps` 保留原函数元信息（见 [装饰器](/python/500-Decorator)）。
4. 类型标注与签名相辅相成：`singers: tuple[str, ...] = ()` 精确表达了"不定长但同型"，比裸 `*args` 更能让调用方与静态检查器安心（类型体系见 [类型注解与 mypy](/python/530-TypeAnnotationMypy)）。
