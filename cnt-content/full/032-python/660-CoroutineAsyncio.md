...
- [Python 基础数据类型：从对象模型到工程实践的深度解析](/python/490-DataTypeObjectModelDeepDive)：建议先完成前一篇的学习
...
```python
# 在协程内部获取当前正在运行的事件循环
# 没有运行中的循环时抛 RuntimeError——这正是想要的防护
loop = asyncio.get_running_loop()
```

**基本写法：获取当前事件循环（旧接口，注意兼容）**
`asyncio.get_event_loop()`
```python
# 3.12 起弃用"没有循环就隐式新建"的行为并发出告警；
# 3.14 起没有正在运行的事件循环时直接抛 RuntimeError。
# 新代码一律 asyncio.run() 启动，协程内部用 get_running_loop()
loop = asyncio.get_event_loop()  # 仅在协程内部才等价于 get_running_loop()
```

**旧写法迁移：设置事件循环策略（3.14 已移除）**
`asyncio.set_event_loop_policy(<策略>)`
```python
# 事件循环策略系统已在 3.14 移除，不要再写这类代码：
# if sys.platform == "win32":
#     asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
# 现代做法：Windows 默认 Proactor 循环已覆盖绝大多数场景；
# 追求更高性能时在非 Windows 平台使用 uvloop：
# import uvloop
# uvloop.run(main())
```
