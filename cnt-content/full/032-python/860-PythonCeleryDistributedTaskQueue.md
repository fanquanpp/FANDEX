...
- [Python 与 Redis](/python/840-PythonRedis)：建议先完成前一篇的学习
...
- **Celery 6.x**：以原生 `asyncio` 支持与 Canvas 改进为目标的下一代版本，发布状态请以官方发布说明为准。
...
- `python/840-PythonRedis`: Redis 作为 Broker 与 Backend
- `python/850-PythonMessageQueue`: AMQP 协议与消息中间件
- `python/790-PythonDocker`: 容器化部署 Celery
- `python/890-PythonDjango`: Django 项目中集成 Celery
