...
- [函数详解](/python/028-FunctionDetailed)：掌握默认参数、返回值与作用域，本篇在其上补齐"可变参数"这最后一块。
- [内置数据结构](/python/040-BuiltinDataStructure)：理解序列与映射两类容器，是 `*`/`**` 解包的前提。
- [推导式与生成器](/python/054-ComprehensionGenerator)：解包常与推导式组合出简洁的装配代码。
...
3. 装饰器与代理函数转发参数时才写 `def wrapper(*args, **kwargs): return f(*args, **kwargs)`，并配合 `functools.wraps` 保留原函数元信息（见 [装饰器](/python/034-Decorator)）。
4. 类型标注与签名相辅相成：`singers: tuple[str, ...] = ()` 精确表达了"不定长但同型"，比裸 `*args` 更能让调用方与静态检查器安心（类型体系见 [类型注解与 mypy](/python/064-TypeAnnotationMypy)）。
