...
related:
  ...
  - 'python/101-ProfilingOptimization'
prerequisites:
  - 'python/014-MultiprocessingMultithreading'
---
...
- [多进程与多线程](/python/014-MultiprocessingMultithreading)：理解 GIL 的作用范围，C 扩展的线程行为与本篇直接相关。
- [性能剖析与优化](/python/101-ProfilingOptimization)：先有剖析结果再谈扩展，本篇第 1 节直接复用其方法论。
- [Python 性能优化](/python/039-PythonPerformance)：向量化与算法优化是 C 扩展之前的两道关卡。
...
1. 直接分发 .c 源码意味着用户机器必须有编译器；正确做法是构建各平台的 wheel（`.so`/`.pyd` 已编好），发布流程见 [包发布](/python/060-PackagePublish) 与 [打包演进](/python/045-PythonPackagingEvolution)。
...
3. **C 扩展长时间持有 GIL**：纯计算段不释放 GIL，多线程调用的收益归零（见 [GIL 与自由线程](/python/102-GILAndFreeThreading)）。计算前后用 `Py_BEGIN_ALLOW_THREADS`/`Py_END_ALLOW_THREADS` 包住不触碰 Python 对象的段落。
