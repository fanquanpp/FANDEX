...
> **本篇与 029 专项的分工**：Part B 只给你"跑通第一版 PWA"的最小骨架（Manifest + 注册 Service Worker + 一两个缓存策略）；Service Worker 生命周期的完整讲解、Cache Storage 细节、客户端管理、安装体验等进阶内容统一放在 `html5/300-ServiceWorkerPWA` 专项，读完 Part B 后应接着学它。两篇互补，不重复展开。
...
- PWA 深化：`html5/300-ServiceWorkerPWA` 完整生命周期与缓存策略；
- 离线存储：`html5/240-HTML5OfflineStorageWebAPI` 本地存储与 Cache API；
- 推送通知：`html5/240-HTML5OfflineStorageWebAPI` 中 Notification API；
