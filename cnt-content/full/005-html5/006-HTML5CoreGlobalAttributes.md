...
`data-*` 值得单独点名：它是"给元素挂自定义数据"的标准通道（如商品卡片上挂 `data-id`），本模块 `html5/034-CustomDataAttribute` 有完整专项。
...
到这里，0 基础前置四件套完成：注释与实体、块级与行内、div/span 容器、id/class/style。接下来 `html5/007-HTML5OverviewCoreFeature` 快速上手课里的所有代码，你已经没有陌生概念了。`data-*` 的完整用法（读取、事件委托、与 JS 框架的关系）见 `html5/034-CustomDataAttribute` 专项。
