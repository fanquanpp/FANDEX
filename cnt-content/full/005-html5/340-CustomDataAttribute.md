...
- 属性操作：`javascript/410-DOMOperationEvent` 中 `getAttribute`/`setAttribute` 全解；
- 事件委托：`javascript/410-DOMOperationEvent` 事件冒泡与委托模式；
- 组件实践：`html5/310-WebComponentsPWADevelopment` 自定义元素中 `observedAttributes`；
- 测试钩子：`javascript/690-JavaScriptProjectPractice` 中 `data-testid` 的 E2E 约定。
