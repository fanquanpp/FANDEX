...
本课程的文档统一采用 UTF-8 编码（详见 `html5/016-MetadataCharacterEncoding`），中文、日文、数学符号等绝大多数字符都可以直接写进源文件，不需要实体。**真正必须用实体的只有三类**：
...
1. 打开 `html5/002-HTML5EnvSetupFirstPage` 创建的 `index.html`，给标题加一行注释，刷新页面确认注释没有显示；
...
注释和实体字符解决的是"字符怎么打"的问题。下一篇 `html5/004-HTML5BlockVsInline` 解决"标签怎么排"的问题：为什么 `<h1>` 独占一行，而 `<span>` 不换行。
