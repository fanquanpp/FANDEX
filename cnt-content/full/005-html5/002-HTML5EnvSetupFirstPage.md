...
你已经完成了网页制作的完整闭环。接下来 001-004 四篇前置课会依次补上零基础必懂的概念：注释与特殊字符（001）、块级与行内元素（002）、div 与 span 容器（003）、id/class/style 全局属性（004）；然后 `007-HTML5OverviewCoreFeature` 会用 5 分钟写第二个网页，并拆解刚才那几行代码背后的含义。
