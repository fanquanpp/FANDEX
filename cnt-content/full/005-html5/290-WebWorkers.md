...
- 离线场景：`html5/300-ServiceWorkerPWA` 中 Service Worker（Worker 家族的一员）；
- 数据缓存：`html5/240-HTML5OfflineStorageWebAPI` 中 Worker 与 IndexedDB 的配合；
- 性能：`javascript/500-DebugPerformanceOptimization` 用性能面板分析主线程任务；
- 并发模式：`javascript/280-AsyncConcurrencyControl` 对比异步任务与 Worker 的取舍。
