...
- 真实异步任务的进度来源见 `html5/320-WebSocket`（实时推送）与 XHR/fetch 的进度事件。

## 9. 扩展学习

- 无障碍：`html5/180-Accessibility` 中 ARIA 的 `progressbar`/`meter` 角色与 `aria-valuenow`；
- 动画：`css/330-CSSAnimationTransition` 让进度变化更平滑；
- 组件化：`html5/310-WebComponentsPWADevelopment` 封装自定义进度条组件；
- 实时更新：`html5/320-WebSocket` 中上传/下载进度的真实数据来源。
