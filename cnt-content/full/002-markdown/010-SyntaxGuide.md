...
1. `020-HeadingSyntax`（标题）
2. `030-ParagraphLineBreak`（段落与换行）
3. `040-BasicTextFormat`（加粗/斜体等基础格式）
4. `060-ListSyntax`（列表）
5. `050-LinkImage`（链接与图片）
...
- `100-Table`（表格）
- `090-CodeBlockSyntaxHighlight`（代码块与语法高亮）
- `070-BlockquoteNestedList`（引用与嵌套列表）

### Layer 2 - 专业层（按需查阅，不急着学）

- `240-CommonMarkSpec`（CommonMark 规范：解析器原理，属于高级话题，零基础可完全跳过）
- `210-LaTeXMathFormula`、`220-Mermaid`（数学公式、图表）
- `260-FrontmatterYAML`、`270-SpecDocumentWriting`、`310-AdvancedSyntaxDocumentAutomation`（工程化）
...
| 2009  | GitHub 发布 GitHub Flavored Markdown（GFM）方言 |
| 2014  | CommonMark 项目启动，开始制定无歧义规范   |
| 2017  | GFM 规范文档发布（基于 CommonMark）       |
| 2020+ | 扩展生态繁荣：任务列表、脚注、数学公式、Mermaid、警报块等 |
...
| **高亮** (第三方扩展，GitHub 不支持) | `<mark>高亮文本</mark>`（HTML 标签可用） | <mark>高亮文本</mark> |
| **脚注** (扩展语法)   | `[^脚注]` 并在文档末尾定义 `[^脚注]: 脚注内容` | [^脚注]           |
...
使用 `-`/`*`/`+` + 空格开头，三者效果完全一致，推荐统一使用 `-`。嵌套子项缩进到**父项内容列**（`- ` 项为 2 空格），详见 `markdown/060-ListSyntax`。
...
使用 `数字 + 英文句号 + 空格` 开头，首项数字决定起始序号、后续数字不影响渲染，推荐按顺序书写。嵌套子项缩进到父项内容列（`1. ` 项为 3 空格），详见 `markdown/060-ListSyntax`。
...
**问题描述**：嵌套列表或引用显示异常。
**原因分析**：嵌套元素缩进不正确。
**解决方案**：子项缩进到父项内容的起始列（无序列表 2 空格、有序列表 3 空格），并全程使用空格而非制表符，详见 `markdown/060-ListSyntax`。
### 16.4. 特殊字符问题
