...
本文是「Git」模块的第 1 篇，难度定位为入门。重点内容：Git 的核心概念、分布式版本控制原理与基本工作流。
...
分支是指向特定提交的指针，允许开发者在独立的环境中开发新功能或修复 bug，而不影响主分支的稳定性。新仓库的默认分支由 `init.defaultBranch` 决定，现行约定为 `main`（旧版本默认 `master`）。
**分支操作**：
