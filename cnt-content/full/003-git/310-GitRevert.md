...
```bash
# 撤销连续区间（左开右闭：不含 abc1234 本身）
git revert abc1234..def5678
# 要包含起点则写 abc1234^..def5678

# 撤销多个不连续的提交（按给定顺序逐个反向）
git revert abc1234 def5678 8c9d0e1
```

### 2.4 修改 revert 消息

revert 默认生成 `Revert "<原提交标题>"` 格式的消息。想自定义：

```bash
git revert -e abc1234              # 提交前打开编辑器修改消息
# 或不自动提交，手工整理后提交：
git revert -n abc1234
git commit -m "revert: 回退认证功能（原因：...）"
```
...
```bash
# 错误提交已推送：revert 消息默认引用原提交，补充说明用 -e 编辑
git revert -e wrong-commit
# 消息模板：
#   Revert "feat: wrong feature"
...
```bash
# 撤销 abc1234（不含）之后到 def5678（含）的提交
git revert abc1234..def5678;
```
