...
在提交时使用 `--fixup` 预标记（消息自动生成为 `fixup! <目标提交标题>`，无需手写）：
...
```
pick   abc1234 feat: add login form
fixup  def5678 fixup! feat: add login form  # 自动排位并标记为 fixup
```
...
（reflog 同样是兜底，备份分支的价值在于「有一眼可辨的名字」。）

## 小结

**初学者要点**

- `git rebase -i <基准>` 打开提交清单，从上到下即从旧到新；改指令保存即执行。
- 高频三板斧：`reword` 改消息、`squash` 合并（保留并合并消息）、`fixup` 合并（丢弃消息）。
- `edit` 暂停点是拆分/修补提交的入口：`reset HEAD^` 拆，`--amend` 补，`--continue` 收。

**进阶注意**

- squash/fixup 的目标永远是**上一行**提交，首行必须 pick；autosquash 让「随手 fixup」的提交自动归位。
- `x/exec` 可在每个提交后运行测试（如 `x npm test`），把「每步都可用」变成机械保证。
- 黄金法则同样适用：只 rebase 未共享的提交，推送用 `--force-with-lease`；rerere 能让重复冲突自动化解。
