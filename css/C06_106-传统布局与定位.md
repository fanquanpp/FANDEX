...
```css
.layer-1 { position: absolute; z-index: 1;  background: rgba(231,76,60,0.7); }
.layer-2 { position: absolute; z-index: 10; background: rgba(52,152,219,0.7); }
.layer-3 { position: absolute; z-index: 5;  background: rgba(46,204,113,0.7); }
```
...
> ⚠️ **常见坑**：子元素的 `z-index` 再高，也无法超越父级层叠上下文的限制。
...
|:--|:--|:--|:--|:--|
| absolute + transform | ❌ | IE10+ | 少 | 通用 |
| absolute + 负 margin | ✅ | IE6+ | 中 | 已知尺寸 |
| absolute + margin:auto | ✅ | IE8+ | 少 | 已知尺寸 |
| table-cell | ❌ | IE8+ | 多 | 兼容旧浏览器 |
| Flexbox | ❌ | IE10+ | 少 | 现代项目首选 |
| Grid | ❌ | 现代浏览器 | 最少 | 最新项目 |
