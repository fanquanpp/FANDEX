...
本项目实现一个功能丰富的交互式表单应用，综合运用 HTML5 表单验证、Canvas 绘图、Web Storage、Geolocation、Drag and Drop 等 Web API。涵盖从表单设计到数据可视化的完整流程。
...
.app-header h1 { font-size: 1.8rem; font-weight: 700; text-align: center; margin-bottom: 20px; }
...
.review-label { font-weight: 500; color: var(--text-secondary); }
    ...
    inputs.forEach((input) => {
        ...
        if (!input.checkValidity()) {
            ...
            if (errorEl) {
                ...
                else if (input.validity.tooShort) errorEl.textContent = `Minimum ${input.minLength} characters`;
...
```javascript
element.addEventListener("dragstart", (e) => { /* 记录拖拽元素 */ });
